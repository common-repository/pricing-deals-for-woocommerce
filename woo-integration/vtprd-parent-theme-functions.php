...
*/
 ...
 //error_log( print_r(  '$post->ID = ' .$post->ID , true ) );      
    ...
    if ( ( ( (in_the_loop() ) || 
     ...
     //error_log( print_r(  'shortcode skip 003 ', true ) );        
          ...
          if ($show_all_rules_msgs > '') {
            ...
            //v2.0.2.02  added new data chain logic, as Globals are sometimes getting lost
            vtprd_set_messaging_transient_data_chain(); //v2.0.2.02
            
            $vtprd_apply_rule_messaging->vtprd_manage_customer_rule_tests($i, $use_messaging_data_chain=TRUE); //v2.0.2.01,   //v2.0.2.02  added messaging variable

            if ($vtprd_apply_rule_messaging->vtprd_test_if_inPop_product($i, $k, $use_messaging_data_chain=TRUE)) {         //v2.0.2.02  added messaging variable
               //error_log( print_r(  'If inpop found, msg loaded ', true ) );
               $loop_msgs_array[] = $i;                
            } else {
            $inPop_found = FALSE;
            if ($vtprd_apply_rule_messaging->vtprd_test_if_actionPop_product($i, $k, $inPop_found, $use_messaging_data_chain=TRUE)) {        //v2.0.2.02  added messaging variable
               //error_log( print_r(  'If actionnpop found, msg loaded ', true ) );
               $loop_msgs_array[] = $i;                
            }              
            
            vtprd_get_messaging_data_chain(); //v2.0.2.02
    ...
    if ( (in_the_loop() ) ||
     ...
     //v1.1.0.5 end  
      ...
      $post->ID =  wp_kses($post->ID ,$allowed_html); //v2.0.3 
 ...
 //error_log( print_r(  'shortcode skip 006 ', true ) );          
      ...
      if ($roles > ' ') {
        $userRole = vtprd_get_current_user_role();
        //mwn04142014
        if ($userRole ==  'notLoggedIn') {  //v2.1.0  the equal test had used a SINGLE EQUAL, not a double as required  
 ...
 //error_log( print_r(  'current user role=  ' .$userRole_name, true ) );  
        ...
        //v2.1.0 begin
        //make both input role names and logged in role name lower case
...
//echo 'incoming attributes= ' .$atts. '<br>'; //mwnt 
  ...
  add_shortcode('pricing_deal_product_msgs','vtprd_pricing_deal_product_msgs');
    ...
    for($y=0; $y < $sizeof_msg_array; $y++) {
      $gotten_id = wpsc_the_product_id();   //v2.0.3
      $message = stripslashes($vtprd_info['product_session_info']['product_rule_full_msg_array'][$y]);   //v2.0.3
      $allowed_html = vtprd_get_allowed_html(); //v2.0.3
      ?>
				<p class="pricedisplay <?php echo wp_kses($gotten_id ,$allowed_html ); ?> vtprd-single-product-msgs"><?php echo wp_kses($message ,$allowed_html ); ?> </p>
      <?php
    } 
...
//	$you_save = wpsc_currency_display ( $you_save );
 ...
 // if ( $vtprd_setup_options['show_old_price'] == 'yes' ) {
	if (($output_old_price) && ($old_price_text > ' ')) {
		$message = $old_price_before . $old_price_text . $old_price_after . "\n";
        echo wp_kses($message ,$allowed_html ); //v2.0.3
  }
	if ( $output_price )
		$message = $price_before . $price_text . $price_after . "\n";
        echo wp_kses($message ,$allowed_html ); //v2.0.3

 // if ( $vtprd_setup_options['show_you_save'] == 'yes' ) {
	if ($output_you_save) {
  	if ($you_save_text > ' ') {
      $message = $you_save_before . $you_save_text . $you_save_after . "\n";
      echo wp_kses($message ,$allowed_html ); //v2.0.3
    } else  
    if ($vtprd_info['product_session_info']['show_yousave_one_some_msg'] > ' ') {
      $message = $vtprd_info['product_session_info']['show_yousave_one_some_msg'] . "\n";
      echo wp_kses($message ,$allowed_html ); //v2.0.3
    }
  }  
...
/* ************************************************
  ...
  *************************************************** */  
  function vtprd_show_product_list_price($product_id=null) {
    ...
    if ($amt) {
      ?>
				<p class="pricedisplay <?php echo wp_kses($gotten_id ,$allowed_html ); ?>"><?php _e('Old Price', 'wpsc'); ?>: <span class="oldprice" id="old_product_price_<?php echo wp_kses($gotten_id ,$allowed_html ); ?>"><?php echo wp_kses($amt ,$allowed_html ); ?></span></p>
      <?php
    } else {
      //original code from wpsc-single_product.php
      $amt = wpsc_product_normal_price(); //v2.0.3
      ?>
  ...
  *************************************************** */
    ...
    if ($pct) {
      ?>
				<p class="pricedisplay product_<?php echo wp_kses($gotten_id ,$allowed_html ); ?>"><?php _e('You save', 'wpsc'); ?>: <span class="yousave" id="yousave_<?php echo wp_kses($gotten_id ,$allowed_html ); ?>"><?php echo wp_kses($amt ,$allowed_html ); ?>! (<?php echo wp_kses($pct ,$allowed_html ); ?>%)</span></p>
			<?php
    } else {
      //original code from wpsc-single_product.php
      ?>
        ...
        <?php if(wpsc_product_on_special()) : ?>
					<p class="pricedisplay product_<?php echo wp_kses($gotten_id ,$allowed_html ); ?>"><?php _e('You save', 'wpsc'); ?>: <span class="yousave" id="yousave_<?php echo wp_kses($gotten_id ,$allowed_html ); ?>"><?php echo wpsc_currency_display(wpsc_you_save('type=amount'), array('html' => false)); ?>! (<?php echo wp_kses($yousave ,$allowed_html ); ?>%)</span></p>
				<?php endif; ?>
  ...
  function vtprd_get_single_product_you_save_pct($product_id=null){
   ...
   //  only applies if one rule set to $rule_execution_type_selected == 'display'.  Carried in an option, set into info...     
    ...
    function vtprd_rule_messaging_load_dummy_cart($product_id) {
      ...
      $allowed_html = vtprd_get_allowed_html(); //v2.0.3 
      $product_id =  wp_kses($product_id ,$allowed_html); //v2.0.3 
      $vtprd_info['parent_plugin_taxonomy'] =  wp_kses($vtprd_info['parent_plugin_taxonomy'] ,$allowed_html); //v2.0.3 
      $vtprd_info['rulecat_taxonomy'] =  wp_kses($vtprd_info['rulecat_taxonomy'] ,$allowed_html); //v2.0.3
  ...
  //Used in fix for when using shortcde, the globals will occasionally not retain their data.
  // copy of vtprd_set_transient_data_chain with zeroes supplied for this case.
  //************************* 
	function vtprd_set_messaging_transient_data_chain() {
  ...
  // copy of vtprd_set_transient_data_chain with zeroes supplied for this case.
  //*************************    
   function vtprd_get_messaging_data_chain() {

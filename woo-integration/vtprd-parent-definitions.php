    ...
    //    'woo_variation_name_list_by_id'  => '',     //used in auto add function ONLY
        ...
        'data_update_options_done_array'  => array ( 
            'required_updates'  => array (
                ...
                '2.0.3 Ruleset conversions'   => true     //v2.1.0            
            ),
  ...
  function  vtprd_get_purchaser_ip_address() {    //v2.0.2.0 renamed
    ...
    */
    ...
    if ($supplied_IP) {
      ...
      $supplied_IP = str_replace( 'https', '', $supplied_IP  ?? '' );    //v2.0.3 
      $supplied_IP = str_replace( 'http', '', $supplied_IP  ?? '' );     //v2.0.3 
      $supplied_IP = str_replace( '://', '', $supplied_IP  ?? '' );      //v2.0.3 
      return $supplied_IP;
    }
    //v2.0.0.9 end
    ...
    if ( version_compare( $php_version, '5.3.1', '<' ) ) {
      ...
      */
      ...
      if (isset($_SERVER['HTTP_CLIENT_IP'])) {
        $client_ip =     sanitize_url($_SERVER['HTTP_CLIENT_IP']);
      } else {
        $client_ip = null;
      }
      
      if (isset($_SERVER['HTTP_X_FORWARDED_FOR'])) {
        $forwarded_for = sanitize_url($_SERVER['HTTP_X_FORWARDED_FOR']);
      } else {
        $forwarded_for = null;
      }
      
      if (isset($_SERVER['REMOTE_ADDR'])) {
        $remote_addr =   sanitize_url($_SERVER['REMOTE_ADDR']);
      } else {
        $remote_addr = null;
      }      
      ...
      if (!empty($client_ip))   
        {
          $vtprd_ip_address = $client_ip;
        }
      //whether ip is from proxy
      elseif (!empty($forwarded_for))  
        {
          $vtprd_ip_address = $forwarded_for;
        }
      //whether ip is from remote address
      else
        {
          $vtprd_ip_address = $remote_addr;
        }
       //v2.0.3 end
      ...
      $vtprd_ip_address = str_replace( 'https', '', $vtprd_ip_address  ?? '' );    //v2.0.3 

<?php
  ...
  ******************************************************** */
	function vtprd_load_vtprd_cart_for_processing(){      //done
   ...
   //error_log( print_r(  'BEGIN vtprd_load_vtprd_cart_for_processing', true ) );
     ...
     // from Woocommerce/templates/cart/mini-cart.php  and  Woocommerce/templates/checkout/review-order.php
      ...
      //$woocommerce_cart_contents = $woocommerce->cart->get_cart(); //v2.1.0 replaced with below
      $woocommerce_cart_contents = WC()->cart->get_cart(); //v2.1.0 
      
      $calculator_product_in_cart = false; //v1.1.1
      $addons_product_in_cart     = false; //v1.1.1
      
      if ( (isset($woocommerce_cart_contents)) &&   //v2.0.3
           (is_array($woocommerce_cart_contents)) &&   //v2.0.3
           (sizeof($woocommerce_cart_contents) > 0) ) {  
      ...
      //error_log( print_r(  'vtprd_unique_cart_id 001 = ' .$_SESSION['vtprd_unique_cart_id'], true ) );    
          ...
          //v2.1.0 begin - use  data_chain  to grab previous cart
          /*
          $vtprd_previous_cart = array(); //if nothing in data_chain
          $data_chain = vtprd_get_transient_cart_data ( 'data_chain' );
          if ($data_chain) {
            $data_chain      = unserialize($data_chain);
            if (isset($data_chain[1])) {  
              $vtprd_previous_cart      = $data_chain[1];
            }        
          }
          */
          //v2.1.0 end           
          ...
          $vtprd_cart->save_transient_cart_id = sanitize_key($_SESSION['vtprd_unique_cart_id']); //v2.0.3 - the session cart_id disappears in post purchase thankyou, so stored here.  
          ...
          foreach ( $woocommerce_cart_contents as $cart_item_key => $cart_item ) {
			...
			if ( ($_product->exists() ) && ($cart_item['quantity'] > 0) ) {
                ...
                //error_log( print_r(  'cart_item in foreach loop', true ) );
                //error_log( var_export($cart_item, true ) );

               //v2.1.0 begin - moved here from below, to allow test for product type to occur earlier
              ...
              //if ($cart_item['variation_id'] > ' ') {  //v2.1.0  simple products now can have a '0' in variation_id 
              if ($cart_item['variation_id'] > '0') {    //v2.1.0       
              ...
              //v2.1.0 - need to KEEP this  here!!
              $product = wc_get_product( $vtprd_cart_item->product_id ); //v1.0.7.4 //v1.1.7 replace get_product with wc_get_product
              
                            
              //v2.1.0 begin - moved above to allow test for product type to occur earlier
              /*
              //v1.1.8.3 begin 
              //to turn on:  add_filter( 'vtprd_disallow_product_types', function() { return array('donation'); } );
          	  // to list more than 1, list using: array('xxx','yyyy','zzz')
              ...
              */
              ...
              //*****************
              /*
               Sometimes, but rarely, $vtprd_cart_item comes back from
                   vtprd_maybe_get_product_session_info
               as EMPTY.
               
               I suspect the issue is in
               function vtprd_get_product_session_info
               where $vtprd_cart, $vtprd_cart_item
               are 'flattened' and stored off, then returned
               
               even though function vtprd_get_product_session_info
               is run in multiple places, the issue seems only to occur here.
               
               So I'm going to store off all 3 here, just for coverage...
              */ 
               $vtprd_hold_cart = $vtprd_cart;
               $vtprd_hold_cart_item = $vtprd_cart_item;
               //v2.1.0 end
              ...
              //v2.1.0 begin
              //*****************
               $vtprd_cart = $vtprd_hold_cart;
               $vtprd_cart_item = $vtprd_hold_cart_item;
               //v2.1.0 end
        ...
        */
              ...
              if ( ( isset($vtprd_info['product_session_info']['product_yousave_total_amt']) ) &&
                 ...
                 //  $vtprd_cart_item->unit_price             =  vtprd_compute_current_unit_price($product_id, $cart_item, $vtprd_cart_item, $product, $vtprd_previous_cart);     //v1.0.8.0 
                  ...
                  $vtprd_previous_cart = array();  //v2.1.0  - changed to  array();
              ...
              $oldprice =  vtprd_get_taxation_price_cart($vtprd_cart_item->product_id, $vtprd_cart_item->unit_price); //v2.1.0
              $_SESSION['vtprd_orig_cart_price_with_taxation_'.$vtprd_cart_item->cart_item_key] = $oldprice;  //v2.1.0 used in subtotal crossout display    
              
               //error_log( print_r(  ' vtprd_orig_cart_price_ = ' .$vtprd_cart_item->unit_price. ' for item_key= ' .$vtprd_cart_item->cart_item_key , true ) );
               //error_log( print_r(  ' vtprd_orig_cart_price_with_taxation_ = ' .$oldprice. ' for item_key= ' .$vtprd_cart_item->cart_item_key , true ) );         
              ...
              ************************************ */
              ...
              $cart_item['product_id'] =  wp_kses($cart_item['product_id'] ,$allowed_html); //v2.0.3 
              $vtprd_info['parent_plugin_taxonomy'] =  wp_kses($vtprd_info['parent_plugin_taxonomy'] ,$allowed_html); //v2.0.3 
              $vtprd_info['rulecat_taxonomy'] =  wp_kses($vtprd_info['rulecat_taxonomy'] ,$allowed_html); //v2.0.3
		   ...
		   } // end if ( ($_product->exists() ) && ($cart_item['quantity'] > 0) ) {
        } //	endforeach;
...
//************************************* 
   ...
   //************************************* 
    ...
    function vtprd_compute_current_unit_price($product_id, $cart_item, $vtprd_cart_item, $product, $vtprd_previous_cart, $price=null){ //v1.0.9.0 added $price=null ==> $price only comes from one place... 
     ...
     /*
        error_log( print_r(  ' ', true ) );
        error_log( print_r(  'funtion vtprd_compute_current_unit_price, $product_id= ' .$product_id, true ) );
      ...
      //v1.0.9.0 end
        ...
        if ( ( get_option( 'woocommerce_calc_taxes' ) == 'no' ) ||
          ...
          // $unit_price  =  vtprd_get_price_including_tax($product_id, $price);
           $qty = intval(1);          //v2.1.0  - added intval for php 8      
   ...
   //************************************* 
	function vtprd_get_line_subtotal_for_lifetime_only($product_id, $vtprd_previous_cart) {      //done
      $lifetime_line_subtotal = 0;
      
      $sizeof_cart_items = is_array($vtprd_previous_cart->cart_items) ? sizeof($vtprd_previous_cart->cart_items) : 0; //v2.1.0
   ...
   //************************************* 
	function vtprd_count_other_coupons(){     //done
      ...
      */
      if(!isset($_SESSION)){
        ...
        //header("Pragma: no-cache");          //v2.0.2.02("Pragma: no-cache");
      }
     ...
     // $vtprd_rules_set[0]->coupons_amount_without_rule_discounts = $coupon_cnt;   //v1.1.0.9
      ...
      //*********************************************************************************************
      $contents = sanitize_text_field($_SESSION['do_no_actions']);
      unset( $_SESSION['do_no_actions'], $contents );
      //*********************************************************************************************
      //v2.0.0.5a end 
     session_write_close(); //v2.0.2.02         
     return;    
  } 
   ...
   ************************************************** */  
    function vtprd_remove_any_other_woo_coupons() {   //done
   ...
   **  v1.1.7.2 grpD NEW FUNCTION
   ************************************************** */  
    function vtprd_remove_coupon_applied_message() {  //done
      ...
      if ( version_compare( WC_VERSION, '3.0.0', '>=' ) ) { 
          ...
          */ 
          if(!isset($_SESSION)){
            session_start();
            //header("Pragma: no-cache");          //v2.0.2.02("Cache-Control: no-cache");          //v2.0.2.02
            //header("Pragma: no-cache");          //v2.0.2.02("Pragma: no-cache");
          }
        ...
        $sizeof_messages = is_array($woocommerce->messages) ? sizeof($woocommerce->messages) : 0; //v2.1.0  
  ...
  //**************************************
	add_filter('woocommerce_coupon_message', 'vtprd_maybe_remove_coupon_message' , 10,3); 
  function vtprd_maybe_remove_coupon_message($msg, $msg_code, $var){     //done
   //error_log( print_r(  'Function begin - vtprd_maybe_remove_coupon_message', true ) );   
    ...
    if ( (isset ($_SESSION['remove_coupon_message'])) &&
         (sanitize_text_field($_SESSION['remove_coupon_message'])) ) {
  ...
  */
  function vtprd_load_vtprd_cart_for_single_product_price($product_id, $price){   //done
  ...
  //error_log( print_r(  'function vtprd_load_vtprd_cart_for_single_product_price begin, $product_id= ' .$product_id. ' $price= ' .$price, true ) );
      ...
      $cart_id = sanitize_key($_SESSION['vtprd_unique_cart_id']);

      //need to escape any single quotes in the data, so we don't get SQL errors
      $flattened_post = str_replace("'", "%27", $flattened_post ?? ''  );   //v2.0.3 added  ?? ''     
   ...
   /*
   error_log( print_r(  'before call 002, $product_id= ' .$product_id, true ) ); 
   error_log( print_r(  'after get_product, $product = ', true ) );
   error_log( var_export($product, true ) );
   */
  ...
  //error_log( print_r(  'vtprd_load_vtprd_cart_for_single_product_price - price after vtprd_get_current_active_price= ' .$price, true ) );
      ...
      $vtprd_previous_cart = array();  //v2.1.0  - changed to  array();
  /*
   error_log( print_r(  '*just* before call 002, $product = ', true ) );
   error_log( var_export($product, true ) );  
  error_log( print_r(  'call 002 to vtprd_compute_current_unit_price', true ) ); 
  */   
      $price  =  vtprd_compute_current_unit_price($product_id, $cart_item, $vtprd_cart_item, $product, $vtprd_previous_cart, $price);            
      ...
      $allowed_html = vtprd_get_allowed_html(); //v2.0.3 
      $use_this_id =  wp_kses($use_this_id ,$allowed_html); //v2.0.3 
      $vtprd_info['parent_plugin_taxonomy'] =  wp_kses($vtprd_info['parent_plugin_taxonomy'] ,$allowed_html); //v2.0.3 
      $vtprd_info['rulecat_taxonomy'] =  wp_kses($vtprd_info['rulecat_taxonomy'] ,$allowed_html); //v2.0.3
  ...
  //*****************************************
  function vtprd_get_current_active_price($product_id,$product) {	//done
  ...
  //error_log( print_r(  'do get price', true ) );  
        ...
        $price = (float) $product->get_price();   //v2.1.0  - added (float) for php 8
  ...
  //v1.1.1  new function
  //******************************
	function vtprd_maybe_get_sale_price($product_id){   //done
	...
	function vtprd_move_vtprd_single_product_to_session($product_id){    //done
      ...
      /*
      error_log( print_r(  ' ', true ) );
      error_log( print_r(  'function vtprd_move_vtprd_single_product_to_session, $product_id= ' .$product_id, true ) );

      error_log( print_r(  'yousave_total_amt 0  = ' .$vtprd_cart->cart_items[0]->yousave_total_amt, true ) );
      $opt = get_option('woocommerce_calc_taxes');
      error_log( print_r(  'woocommerce_calc_taxes  = ' .$opt, true ) );
      $opt = get_option('woocommerce_prices_include_tax');
      error_log( print_r(  'woocommerce_prices_include_tax  = ' .$opt, true ) );
      $opt = get_option('woocommerce_tax_display_cart');
      error_log( print_r(  'woocommerce_tax_display_cart  = ' .$opt, true ) );
      ...
      */
      ...
      if ($vtprd_info['ruleset_has_a_display_rule'] == 'yes') {      
        ...
        foreach ($vtprd_cart->cart_items[0]->cartAuditTrail as $key => $row) {                 
            if (isset($row['ruleset_occurrence'])) {  //v2.1.0  - $row['ruleset_occurrence'] may not exist - skip in that case
...
//      $product_variations_sw = vtprd_test_for_variations($product_id);
     ...
     if ($vtprd_cart->cart_items[0]->yousave_total_amt > 0) {
      ...
      if (class_exists('WC_Product_Addons')) { 
         ...
         $sizeof_product_addons = is_array($product_addons) ? sizeof($product_addons) : 0; //v2.1.0 
         //if ( is_array( $product_addons ) && sizeof( $product_addons ) > 0 ) { 
         if ( $sizeof_product_addons > 0 ) {
        ...
        if ( (isset ($_SESSION['single_product_first_time_'.$product_id])) &&
             (vtprd_sanitize_text_or_array_field($_SESSION['single_product_first_time_'.$product_id])) &&   //v2.0.3 added line for sanitize_text
    ...
    //v1.0.9.0 end           
      ...
      $_SESSION['vtprd_product_session_info_'.$product_id] = $vtprd_info['product_session_info'];
       ...
       /*           
            error_log( print_r(  ' ', true ) );
            error_log( print_r(  'AT product_session_info LOAD TIME for $product_id= ' .$product_id, true ) );
            error_log( var_export($vtprd_info['product_session_info'], true ) );
            error_log( print_r(  ' ', true ) );
       */
       
      $save_transient_cart_id = $vtprd_cart->save_transient_cart_id; //v2.0.3 - the session cart_id disappears in post purchase thankyou, so stored here. 
      
      //initialize vtprd_cart to clear all discount values...  //v1.0.7.8
      $vtprd_cart = new vtprd_Cart;                            //v1.0.7.8 
      
                                    
      $vtprd_cart->save_transient_cart_id = $save_transient_cart_id; //v2.0.3 - the session cart_id disappears in post purchase thankyou, so stored here. 
    ...
    // *** ------------------------------------------------------------------------------------------------------- ***
    function vtprd_get_cart_html_prices($called_from, $catalog_or_inline=null) {   //v1.1.1 changed to parameter name     //done
    global $post, $wpdb, $woocommerce, $vtprd_cart, $vtprd_cart_item, $vtprd_info, $vtprd_setup_options, $vtprd_rules_set;
 //error_log( print_r(  ' ' , true ) );
 //error_log( print_r(  'vtprd_get_cart_html_prices, $called_from= ' .$called_from. ' $catalog_or_inline= ' .$catalog_or_inline, true ) );    
      ...
      if ($called_from == 'single') {
        ...
        $number_of_times = is_array($vtprd_cart->cart_items) ? sizeof($vtprd_cart->cart_items) : 0; //v2.1.0
      }
           ...
           //v1.0.9.3  begin
            ...
            if ($called_from == 'single')  {         
              if ( (isset($_SESSION['vtprd_product_old_price_'.$product_id])) &&
                   (vtprd_sanitize_text_or_array_field($_SESSION['vtprd_product_old_price_'.$product_id])) ) {  //v2.0.3 added line for sanitize_text
                  $oldprice = vtprd_sanitize_text_or_array_field($_SESSION['vtprd_product_old_price_'.$product_id]['single_product_current_price']);
              ...
              $oldprice = sanitize_text_field($_SESSION['vtprd_orig_cart_price_'.$vtprd_cart->cart_items[$z]->cart_item_key]);
              $oldprice_formatted = wc_price($oldprice); 
    ...
    //       $vtprd_cart->cart_items[$z]->product_catalog_price_displayed_incl_tax_woo =  vtprd_get_price_including_tax($product_id, $oldprice); 
           ...
           if ($vtprd_setup_options['discount_taken_where'] == 'discountUnitPrice') { 
              ...
              $test_total_discount_price                   = round((float)$discount_price * (float)$vtprd_cart->cart_items[$z]->quantity , 2);         //v2.1.0  - added (float) for php 8
    ...
    function vtprd_maybe_price_incl_tax ($product_id, $price) { //done 
       global $woocommerce;    
  ...
  function vtprd_test_for_variations($prod_ID) {     //done
    ...
    //v2.0.0 simplified
     ...
     $attributes = (array) maybe_unserialize( get_post_meta($prod_ID, '_product_attributes', true) );    //maybe_unserialize is a WP function
   ...
   function vtprd_format_money_element($price) {  //done
  ...
  //v2.0.1.0 revamped
  function vtprd_get_current_user_role() {   //done
    global $current_user; 
  ...
  //*******************************************************************************
  function vtprd_print_widget_discount() {    //done
  ...
  *************************************************** */
	function vtprd_print_widget_discount_rows() {   //done
     global $vtprd_cart, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
      $allowed_html = vtprd_get_allowed_html(); //v2.0.3
      $output = '<ul class="cart_list product_list_widget vtprd_product_list_widget">' ; //v2.0.3
      echo wp_kses($output ,$allowed_html ); //v2.0.3
  
      $sizeof_cart_items = is_array($vtprd_cart->cart_items) ? sizeof($vtprd_cart->cart_items) : 0; //v2.1.0
      for($k=0; $k < $sizeof_cart_items; $k++) {  
       	if ( $vtprd_cart->cart_items[$k]->yousave_total_amt > 0) {            
            echo wp_kses('<li>' ,$allowed_html ); //v2.0.3
            $msg_cnt = 0;  
            ...
            if ($vtprd_setup_options['show_cartWidget_discount_details_grouped_by_what'] == 'rule') {
              ...
              foreach($vtprd_cart->cart_items[$k]->yousave_by_rule_info as $key => $yousave_by_rule) {
                $msg_cnt++;
                if ($msg_cnt > 1) {
                  echo wp_kses('</li><li>' ,$allowed_html ); //v2.0.3
                }
      ...
      echo wp_kses('</ul>' ,$allowed_html ); //v2.0.3

    return;
    
  }
     
	function vtprd_print_discount_detail_line_widget($amt, $units, $k) {     //done
    global $vtprd_cart, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
    $output = ''; //v1.0.7.9
    
    $sizeof_variation_array = is_array($vtprd_cart->cart_items[$k]->variation_array) ? sizeof($vtprd_cart->cart_items[$k]->variation_array) : 0; //v2.1.0
    if ($sizeof_variation_array > 0   ) {
      ...
      if ($vtprd_cart->cart_items[$k]->variation_name_html > '')  {
        ...
        foreach($vtprd_cart->cart_items[$k]->variation_array as $key => $value) {          
          //v1.0.7.8  begin               
          $name  = str_replace( 'attribute_pa_', '', $key  ?? '' );  //post v 2.1                                         //v2.0.3 added  ?? '' 
          $name  = str_replace( 'attribute_', '', $key  ?? '' );     //post v 2.1   for on-the-fly variations             //v2.0.3 added  ?? '' 
          $value = str_replace( 'attribute_', '', $value  ?? '' );   //post v 2.1   for on-the-fly variations             //v2.0.3 added  ?? '' 
          $name  = str_replace( 'pa_', '', $name  ?? '' );   //pre v 2.1                                                  //v2.0.3 added  ?? '' 
          $name  = ucwords(urldecode($name));  //2.1.0  changed  ucwords(  to   ucwords(urldecode
          $current_version =  WOOCOMMERCE_VERSION;
          if( (version_compare(strval('2.0.2.0'), strval($current_version), '>') == 1) ) {   //'==1' = 2nd value is lower     
            //pre 2.1
	...
	function vtprd_print_widget_discount_total() {  //done
	...
	function vtprd_print_widget_new_combined_total() {     //done
    global $vtprd_cart, $woocommerce, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
  ...
  *************************************************** */
	...
	function vtprd_print_widget_purchases_subtotal() {    //done
    global $vtprd_cart, $woocommerce, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
  ...
  *************************************************** */
	...
	function vtprd_print_checkout_discount() {   //done
 ...
 /*
    ...
    */
    ...
    $output = '</table>  <!-- vtprd discounts table close -->  ';
  ...
  **   print discount amount by product, and print total              
  *************************************************** */
	function vtprd_print_cart_discount_rows($execType) {  //done
   global $vtprd_cart, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
  ...
  function vtprd_print_cart_widget_title() {  //done   
	...
	function vtprd_print_discount_detail_line($amt, $units, $execType, $k) {   //done
    global $vtprd_cart, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
    $output = ''; //v1.0.7.9
  ...
  //***************************************
  function vtprd_print_cart_purchases_subtotal($execType) {   //done
   global $vtprd_cart, $woocommerce, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;   
 ...
 //         $subTotal = $vtprd_cart->cart_original_total_amt;    //show as a credit
  ...
  //***************************************
	function vtprd_print_new_cart_checkout_subtotal_line($execType) {    //done
     global $vtprd_cart, $woocommerce, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;   
      ...
      //if ( $woocommerce->cart->tax_display_cart == 'excl' ) {  //v2.0.2.02a
      if (get_option('woocommerce_tax_display_cart')   == 'excl') {  //v2.0.2.02a
    		$subtotal = $woocommerce->cart->subtotal_ex_tax ;
    	} else {
    		$subtotal = $woocommerce->cart->subtotal;
      }  
	...
	function vtprd_print_cart_discount_total($execType) {   //done
   global $vtprd_cart, $woocommerce, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
  ...
  *************************************************** */
	function vtprd_email_cart_reporting($msgType) {    //done ALL EMAIL functions
    ...
    if ($vtprd_setup_options['show_checkout_discount_titles_above_details'] == 'yes') {
      if ($msgType == 'html') {
        ...
        //New //header("Pragma: no-cache");          //v2.0.2.02s, but printed as TD instead, to keep the original structure going...                    
        $output .= '<tr>';
    ...
    if ($msgType == 'html')  {
      ...
      if ($sizeof_variation_array > 0   ) {
        ...
        if ($vtprd_cart->cart_items[$k]->variation_name_html > '')  {
          ...
          $variation_name_html = str_replace( '<p>',  '', $variation_name_html  ?? '' );     //v2.0.3 added  ?? ''
          $variation_name_html = str_replace( '</p>', '', $variation_name_html  ?? '' );     //v2.0.3 added  ?? ''                                                                          
  ...
  *        $msgType = 'html' or 'plainText'            
  *************************************************** */
	function vtprd_thankyou_cart_reporting() {   //done all thankyou functions
  ...
  *        $msgType = 'html' or 'plainText'            
  *************************************************** */
	function vtprd_checkout_cart_reporting($msgType) {      //v1.0.8.0        //done
  ...
  **   Assemble all of the cart discount row info              
  *************************************************** */
	function vtprd_checkout_cart_discount_rows($msgType) {     //done
     global $vtprd_cart, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
       	...
       	if ( ($vtprd_cart->cart_items[$k]->yousave_total_amt > 0) ||  //v1.1.0.6
            ...
            if ($vtprd_setup_options['show_checkout_discount_details_grouped_by_what']   == 'rule') {
              ...
              foreach($vtprd_cart->cart_items[$k]->yousave_by_rule_info as $key => $yousave_by_rule) {
                ...
                //error_log( print_r(  '$yousave_by_rule', true ) );
                //error_log( var_export($yousave_by_rule, true ) );
	...
	function vtprd_checkout_discount_detail_line($amt, $units, $msgType, $k) {     //done
    global $vtprd_cart, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;
      $output = ''; //v1.0.7.9
	...
	function vtprd_checkout_cart_purchases_subtotal($msgType) {      //done 3
   global $vtprd_cart, $woocommerce, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule, $vtprd_setup_options;   
...
*       Exclude list - excludes the product in a rule population 
  ...
  *************************************************** */  
  ...
  function vtprd_fill_include_exclude_lists($prod_id,$details) {  //done 3
      ...
      */
      ...
      if (sizeof($details['includeOrExclude_included_rules']) > 0) {
        ...
        foreach ($details['includeOrExclude_included_rules'] as  $includeOrExclude_included_rules) {                   
          ...
          $temp  .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a class="" target="_blank" href="'.esc_url(VTPRD_URL.'post.php?post=' .$array_rule_id. '&action=edit').'">' .$array_rule_title. '</a>';             
  ...
  function vtprd_maybe_get_product_session_info($product_id) {     //done
     global $vtprd_info;   
  ...
  //error_log( print_r(  'vtprd_disallow_product_types 002, SKIP THIS PRODUCT ', true ) );                    
    ...
    if ( (isset($_SESSION['vtprd_product_session_info_'.$product_id])) &&
         (vtprd_sanitize_text_or_array_field($_SESSION['vtprd_product_session_info_'.$product_id])) ) {    //v2.0.3 added line for sanitize_text  
      $vtprd_info['product_session_info'] = vtprd_sanitize_text_or_array_field($_SESSION['vtprd_product_session_info_'.$product_id]);
      //v2.0.0 begin
  ...
  *************************************************** */
  ...
  function vtprd_get_product_session_info($product_id, $price=null){   //done
  ...
  *************************************************** */
	function vtprd_apply_rules_to_single_product($product_id, $price=null){      //done
 
    global $post, $vtprd_cart, $vtprd_cart_item, $vtprd_info, $vtprd_rules_set, $vtprd_rule;
    
    //v2.0.2.0 begin
    if ( isset($_SESSION['vtprd_unique_cart_id']) ) { 
       $vtprd_saved_unique_cart_id = sanitize_key($_SESSION['vtprd_unique_cart_id']);
    }  else {
  ...
  //*********************************
  function vtprd_maybe_get_price_single_product($product_id, $price=null){   //done
    global $post, $vtprd_info;
  ...
  *************************************************** */	
	function vtprd_save_discount_purchase_log($cart_parent_purchase_log_id) {   //done
      ...
      $sql = $wpdb->prepare( "SELECT * FROM `" . VTPRD_PURCHASE_LOG . "` WHERE `cart_parent_purchase_log_id`= %s LIMIT 1",
             $cart_parent_purchase_log_id                         
            );  
      $vtprd_purchase_log = $wpdb->get_row($sql,ARRAY_A) ; 
      //v2.0.3 end 
      ...
      $wpdb->query(
        $wpdb->prepare( "INSERT INTO `".VTPRD_PURCHASE_LOG."` (`id`,`cart_parent_purchase_log_id`,`purchaser_name`,`purchaser_ip_address`,`purchase_date`,`cart_total_discount_currency`,`ruleset_object`,`cart_object`) 
           VALUES (%s,%s,%s,%s,%s,%s,%s,%s );",
                $next_id,
                $cart_parent_purchase_log_id,
                $vtprd_cart->billto_name,
                $purchaser_ip_address,
                $date,
                $vtprd_cart->yousave_cart_total_amt,
                $ruleset_object,
                $cart_object
            )
       );                
      ...
      //$purchase_log_row_id = $wpdb->get_var($wpdb->prepare("SELECT LAST_INSERT_ID() AS `id` FROM `".VTPRD_PURCHASE_LOG."` LIMIT 1")); //v2.0.3
      $purchase_log_row_id = $wpdb->get_var($wpdb->prepare("SELECT LAST_INSERT_ID() AS `id` FROM `%1s` LIMIT 1",VTPRD_PURCHASE_LOG)); //v2.0.3  %1s prevents prepare from adding single quotes
      ...
      foreach($vtprd_cart->cart_items as $key => $cart_item) {  
        if ($cart_item->yousave_total_amt > 0 ) { 
          ...
          $wpdb->query(
            $wpdb->prepare("INSERT INTO `".VTPRD_PURCHASE_LOG_PRODUCT."` (`id`,`purchase_log_row_id`,`product_id`,`product_title`,`cart_parent_purchase_log_id`,
                `product_orig_unit_price`,`product_total_discount_units`,`product_total_discount_currency`,`product_total_discount_percent`) 
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s );", 
                    ...
                    $cart_parent_purchase_log_id,
                    $cart_item->db_unit_price,
                    $cart_item->yousave_total_qty,
                    $cart_item->yousave_total_amt,
                    $cart_item->yousave_total_pct    
                )
           );                
          //v2.0.3 end                
      
          //$purchase_log_product_row_id = $wpdb->get_var($wpdb->prepare("SELECT LAST_INSERT_ID() AS `id` FROM `".VTPRD_PURCHASE_LOG_PRODUCT."` LIMIT 1"));  //v2.0.3
          $purchase_log_product_row_id = $wpdb->get_var($wpdb->prepare("SELECT LAST_INSERT_ID() AS `id` FROM `%1s` LIMIT 1",VTPRD_PURCHASE_LOG_PRODUCT));   //v2.0.3  %1s prevents prepare from adding single quotes
          ...
          foreach($cart_item->yousave_by_rule_info as $key => $yousave_by_rule) {
            ...
            $wpdb->query(
              $wpdb->prepare("INSERT INTO `".VTPRD_PURCHASE_LOG_PRODUCT_RULE."` (`id`,`purchase_log_product_row_id`,`product_id`,`rule_id`,`cart_parent_purchase_log_id`,
                    `product_rule_discount_units`,`product_rule_discount_dollars`,`product_rule_discount_percent`)                   
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s );", 
                    $next_id,
                    $purchase_log_product_row_id,
                    $cart_item->product_id,
                    $rule_id,
                    $cart_parent_purchase_log_id,
                    $discount_applies_to_qty,
                    $yousave_amt,
                    $yousave_pct  
                  )
             );                
  ...
  *************************************************** */
  function vtprd_recompute_discount_price($variation_id, $price){        //done  2
      global $vtprd_info;  
      
      $yousave_amt = 0;
      $sizeof_pricing_array = is_array($vtprd_info['product_session_info']['pricing_by_rule_array']) ? sizeof($vtprd_info['product_session_info']['pricing_by_rule_array']) : 0; //v2.1.0 
      ...
      for($y=0; $y < $sizeof_pricing_array; $y++) {
        ...
        $sizeof_pricing_rule_applies_to_variations_array = is_array($pricing_rule_applies_to_variations_array) ? sizeof($pricing_rule_applies_to_variations_array) : 0; //v2.1.0 
        if ($sizeof_pricing_rule_applies_to_variations_array > 0) {
  ...
  function vtprd_compute_percent_discount($PercentValue, $price){
   ...
   //error_log( print_r(  'BEGIN vtprd_woo_maybe_create_coupon_types, $coupon_name= ' .$coupon_name. ' $auto= ' .$auto, true ) ); 
      ...
      $coupon_id 	= $wpdb->get_var (
			$wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_title = %s  AND post_type = 'shop_coupon' AND post_status = 'publish'  LIMIT 1",
                 $coupon_name
            )
       );                
       //v2.0.3 end 
   ...
   //error_log( print_r(  'BEGIN vtprd_woo_maybe_delete_all_single_auto_coupons= ', true ) ); 
      ...
      $post_content_like = '%single Automatic Coupon%';
      $sql = $wpdb->prepare( "SELECT post_title  FROM " . $wpdb->prefix . "posts   WHERE   post_type = 'shop_coupon' AND post_status = 'publish' AND  post_content LIKE %s ",
             $post_content_like
            );   
       //v2.0.3 end
 ...
 //error_log( print_r(  'BEGIN  vtprd_woo_maybe_delete_coupon_types', true ) );
  ...
  //v2.0.3 - recoded to remove "if ( ($frontEnd_page", as the function is only executed out of admin rule update
  function vtprd_woo_ensure_coupons_are_allowed() {     
      
    $coupons_enabled = get_option( 'woocommerce_enable_coupons' );
    if ($coupons_enabled == 'no') {
        $message  =  '<strong>' . __('Message from Pricing Deals Plugin => WooCommerce setting "Enable the use of coupons" checkbox must be checked, as Pricing Deal cart discounts are now being applied using the WOO coupon system.' , 'vtprd') . '</strong>' ;
        $message .=  '<br><br>';
        $message .=  '<strong>' . __('"Enable the use of coupons" **has been** reset to Checked.' , 'vtprd') . '</strong>' ;
        $admin_notices = '<div id="message" class="error fade" style="background-color: #FFEBE8 !important;"><p>' . $message . ' </p></div>';
        //v2.0.0.2 begin
        
        add_action( 'admin_notices', function() use ( $admin_notices ) {$allowed_html = vtprd_get_allowed_html(); echo wp_kses($admin_notices ,$allowed_html); }, 10 );
        //v2.0.0.2 end
        update_option( 'woocommerce_enable_coupons','yes');    
    }
    
    /*
    $frontEnd_page = vtprd_sanitize_text_or_array_field($_REQUEST['page']); //v2.0.3
    if ( ($frontEnd_page == 'woocommerce_settings' ) ) {
      $coupons_enabled = get_option( 'woocommerce_enable_coupons' );
      if ($coupons_enabled == 'no') {
...
//error_log( print_r(  'vtprd_format_amt_and_adjust_for_taxes, Begin price= ' .$amt, true ) ); 
  ...
  function vtprd_get_price_including_tax($product_id, $discount_price){ 
    ...
    $price = (float) $discount_price;        //v2.1.0  - added (float) for php 8
    
    //error_log( print_r(  'discount_price after FLOAT =  ' .$price , true ) );
  ...
  //error_log( print_r(  'get_tax_class 007', true ) );
        ...
        //error_log( print_r(  'PRICE at EXIT =  ' .$price , true ) );
...
//error_log( print_r(  'vtprd_get_price_including_tax_forced, Begin price= ' .$price .' Product= ' .$product_id , true ) );
  ...
  function vtprd_coupon_apply_before_tax(){ 
    ...
    function vtprd_my_show_extra_profile_fields( $user ) { 
    						...
    						<tr>
    							...
    							<td>
    								<?php //if ( empty( $user->woocommerce_api_consumer_key ) ) : ?>
    									<input name="vtprd_user_is_tax_exempt" type="checkbox" <?php echo wp_kses($checked ,$allowed_html );//v1.1.0.7  ?> id="vtprd_user_is_tax_exempt" value="yes" />
      ...
      function vtprd_my_save_extra_profile_fields( $user_id ) {
        ...
        if (isset($_REQUEST['vtprd_user_is_tax_exempt'])) {
           $vtprd_user_is_tax_exempt = vtprd_sanitize_text_or_array_field($_REQUEST['vtprd_user_is_tax_exempt'] );        
           update_usermeta( $user_id, 'vtprd_user_is_tax_exempt', $vtprd_user_is_tax_exempt );
  	...
  	function vtprd_get_attributes( $product ) {
		...
		if ( $product->is_type( 'variation' ) ) {
			...
			foreach ( $product->get_variation_attributes() as $attribute_name => $attribute ) {
				...
				$attributes[] = array(
					'name'   => ucwords(urldecode( str_replace( 'attribute_', '', str_replace( 'pa_', '', $attribute_name  ?? '' )  ?? '' ) ) ),  //v2.0.3 added  ?? ''
					'option' => $attribute,
				);   //2.1.0  changed  ucwords(  to   ucwords(urldecode
			}
			...
			foreach ( $product->get_attributes() as $attribute ) {
				...
				$attributes[] = array(
					'name'      => ucwords(urldecode( str_replace( 'pa_', '', $attribute['name']  ?? '' ) ) ),    //v2.0.3 added  ?? ''
  ...
  *************************************************** */
  ...
  function vtprd_maybe_resync_rules_set() {
    ...
    foreach ($vtprd_rules_set as $key => $rule )  { 
       ...
       if ($post) {              
          //v2.0.0 begin
          $sizeof_error_message = is_array($rule->rule_error_message) ? sizeof($rule->rule_error_message) : 0; //v2.1.0 
          switch (TRUE) {
              case ($rule->rule_status == 'pending'):
                  //if pending, do nothing
                break;
              case ($sizeof_error_message > 0):
  ...
  function vtprd_admin_notices() {
    ...
    if ($notices= get_option('vtprd_deferred_admin_notices')) {
      foreach ($notices as $notice) {
        echo wp_kses($notice,$allowed_html ); //v2.0.3
      }
  ...
  function vtprd_check_for_deactivation_action() {
   ...
   if (version_compare(PHP_VERSION, VTPRD_EARLIEST_ALLOWED_PHP_VERSION) < 0) {    //'<0' = 1st value is lower  
        ...
        $message .=  '<br>' . __('You need to be running **at least PHP version 7** to use this plugin. Please contact your host and request an upgrade to PHP 7+ .  Once that has been installed, you can activate this plugin.' , 'vtprd');
        $admin_notices = '<div id="message" class="error fade" style="background-color: #FFEBE8 !important;"><p>' . $message . ' </p></div>';
        //v2.0.0.2 begin
        //add_action( 'admin_notices', create_function( '', "echo '$admin_notices';" ) );
        add_action( 'admin_notices', function() use ( $admin_notices ) {$allowed_html = vtprd_get_allowed_html(); echo wp_kses($admin_notices ,$allowed_html ); }, 10 );
 ...
 //****************************** 
  ...
  function vtprd_check_for_page() {          //done  
    //error_log( print_r(  'Function begin - vtprd_check_for_page', true ) ); 
      ...
      */
      ...
      if ($postID == $woocommerce_cart_page_id) {        
        ...
        vtprd_maybe_send_cart_js_trigger_page_reload();   //v2.0.3
        return;
 ...
 //******************************  
	function vtprd_maybe_send_cart_js_trigger_page_reload() {        //v2.0.3 - changed title
  
   
 //error_log( print_r(  'Function begin - vtprd_maybe_send_cart_js_trigger_page_reload', true ) ); 
   ...
   */
   ...
   //error_log( print_r(  'vtprd_send_cart_js_trigger_page_reload - NEVER', true ) );
    ...
    if ($vtprd_js_trigger_cart_page_reload == 'CouponOnly' ) {
      ...
      $sizeof_vtprd_rule_coupon_activations = is_array($vtprd_rule_coupon_activations) ? sizeof($vtprd_rule_coupon_activations) : 0; //v2.1.0 
      //if (sizeof($vtprd_rule_coupon_activations) == 0) {
      if ($sizeof_vtprd_rule_coupon_activations == 0) {
   ...
   //error_log( print_r(  'vtprd_send_cart_js_trigger_page_reload - SEND JS', true ) );
    ...
    add_action( "wp_enqueue_scripts",     array(&$this, 'vtprd_send_cart_js_trigger_page_reload'),1  );  //v2.0.3

  }

 //******************************
 //v2.0.3 New Function 
 //triggered above, at wp_head execution time
 //******************************  
	function vtprd_send_cart_js_trigger_page_reload() {
      $vtprd_inline_js = '
        ...
        '.
      // Placeholder style.
	  wp_register_script( 'vtprd-inline-js', false );
	  wp_enqueue_script ( 'vtprd-inline-js' );
	  wp_add_inline_script( 'vtprd-inline-js', $vtprd_inline_js );
 ...
 */
 ...
 //******************************  
    ...
    $price =  str_replace($currency_symbol,'',$price_formatted  ?? '');   //v2.0.3 added  ?? ''
    
    $price_suffix = $product->get_price_suffix();
    
    //strip out suffix, if there
    $price =  str_replace($price_suffix,'',$price ?? '' );  //v2.0.3 added  ?? ''
    ...
    $thousands_sep = wp_specialchars_decode( stripslashes( get_option( 'woocommerce_price_thousand_sep' ) ), ENT_QUOTES );
    $price =  str_replace($thousands_sep,'',$price ?? '');    //v2.0.3 added  ?? ''
  ...
  */
    ...
    if ($vtprd_last_admin_update_ts > sanitize_text_field($_SESSION['last_frontend_ts']) ) {
      //grab AUTO-ADD info
...
*/
  ...
  //return true only if discount by Pricing Deals 
    ...
    if ($is_a_variable_product)  { 
      if ( version_compare( WC_VERSION, '3.0.0', '>=' )) {
          ...
          $sizeof_children = is_array($product_visible_children) ? sizeof($product_visible_children) : 0; //v2.1.0     
      }  else {
        $sizeof_test_children = is_array($product_info->children) ? sizeof($product_info->children) : 0; //v2.1.0 
        //if (sizeof($product_info->children) == 0) { 
        if ($sizeof_test_children == 0) {  
          $product_info->get_children();                
        }
        if (isset($product_info->children['visible'])) {
          //$sizeof_children = sizeof($product_info->children['visible']); 
          $sizeof_children = is_array($product_info->children['visible']) ? sizeof($product_info->children['visible']) : 0; //v2.1.0 
        } else {
          //pre woo 2.4
          //$sizeof_children = sizeof($product_info->children);
          $sizeof_children = is_array($product_info->children) ? sizeof($product_info->children) : 0; //v2.1.0 
        } 
      }
  ...
  *************************************************** */
  ...
  function vtprd_build_inline_front_end_css($setup_options = null) { 
    ...
    //error_log( print_r(  'Function begin - vtprd_build_inline_front_end_css', true ) );
      ...
      if ($vtprd_setup_options['discount_taken_where'] == 'discountCoupon') {
        ...
        $coupon_title_hyphenated = str_replace( ' ', '-', $coupon_title  ?? '' );   //v2.0.3 added  ?? ''
     ...
     //error_log( print_r(  '$sizeof_rules_set: ' .$sizeof_rules_set, true ) ); 
        for($i=0; $i < $sizeof_rules_set; $i++) { 
          if ( ($vtprd_rules_set[$i]->only_for_this_coupon_name > ' ') &&
            ...
            $only_for_this_coupon_name_final = str_replace( ' ', '-', $only_for_this_coupon_name_lower  ?? ''  );    //v2.0.3 added  ?? ''   
  ...
  //*************************
  function vtprd_build_rule_deal_info() {    //done
  ...
  //*************************  
	function vtprd_get_transient_cart_data ( $transient_data_type, $cart_id=null ) {
       /*
       error_log( print_r(  'Function begin - vtprd_get_transient_cart_data,  transient_data_type= ' .$transient_data_type. ' cart_id= ' .$cart_id , true ) );
    
        error_log( print_r(  ' ', true ) );
        error_log( print_r(  ' ', true ) );
        error_log( print_r(  '$_SESSION array=', true ) );     
      ...
      */        
      ...
      $transient_object = 
         $wpdb->get_var (
			$wpdb->prepare(
                "SELECT transient_object  FROM `".VTPRD_TRANSIENT_CART_DATA."`   WHERE   `cart_id` = %s  AND   `transient_data_type` = %s ",
                $cart_id,
                $transient_data_type               
			)
		); 
      ...
      $transient_object = str_replace( "%27", "'",  $transient_object ?? ''  );    //v2.0.3 added  ?? ''    
  ...
  //*************************  
	function vtprd_get_transient_cart_id_by_customer_id ( $customer_id, $transient_data_type  ) {
       ...
       //error_log( print_r(  'Function begin - vtprd_get_transient_cart_id_by_customer_id ', true ) );
      ...
      $cart_id 	= $wpdb->get_var (
			$wpdb->prepare(
                "SELECT cart_id  FROM `".VTPRD_TRANSIENT_CART_DATA."`   WHERE   `customer_id` = %s  AND   `transient_data_type` = %s  ",
                $customer_id,
                $transient_data_type               
			)
		);
      ...
      return sanitize_key($cart_id); 
   } 
     
     
  //*************************
  //v2.0.2.0 New Function
  //v2.0.3 reformatted to use customer_id lookup before generating new unique cart_id
  //*************************  
	function vtprd_set_transient_cart_data ( $transient_data_type, $transient_object, $cart_id=null ) {
       
      global $wpdb;
               
       //error_log( print_r(  'Function begin - vtprd_set_transient_cart_data, $transient_data_type= ' .$transient_data_type. ' $cart_id= ' .$cart_id, true ) );       
      
      // $cart_id can be supplied
      if (!$cart_id) {
        if (isset($_SESSION['vtprd_unique_cart_id'])) {
          $cart_id = sanitize_key($_SESSION['vtprd_unique_cart_id']);
          //error_log( print_r(  'cart_id taken from session= ' .$cart_id, true ) );
        }  
      } 
       
      if (isset($_SESSION['vtprd_customer_id'])) {
        $customer_id = sanitize_text_field($_SESSION['vtprd_customer_id']);
      } else {  
        if( is_user_logged_in() ) {
          $customer_id = get_current_user_id(); //v2.0.3
          $_SESSION['vtprd_customer_id'] = $customer_id;  //v2.0.3 
        } else {
          $customer_id = null;
        }
      }  
      ...
      if (!$cart_id) {
         if ($customer_id) {   //v2.0.3 added if
            $cart_id = vtprd_get_transient_cart_id_by_customer_id ( $customer_id, 'customer_id_for_cart_id' );
         }
         //rare case
        if (!$cart_id) {
           vtprd_find_or_set_unique_cart_id(); //v2.0.3
           $cart_id = sanitize_key($_SESSION['vtprd_unique_cart_id']);  //v2.0.3
           //error_log( print_r(  'new cart_id generated= ' .$cart_id, true ) );
        }
      }
      ...
      */
      ...
      //ON DUPLICATE KEY UPDATE `customer_id` = '".$customer_id."', `cart_date` = '".$date."', `transient_object` = '".$transient_object."' ;");
       ...
       $wpdb->query(
			$wpdb->prepare(
                "INSERT INTO `".VTPRD_TRANSIENT_CART_DATA."` (`cart_id`,`transient_data_type`,`customer_id`,`cart_date`,`transient_object`) 
                 VALUES (%s,%s,%s,%s,%s )
                 ON DUPLICATE KEY UPDATE `customer_id` = %s, `cart_date` = %s, `transient_object` = %s ",
                $cart_id, 
                $transient_data_type,
                $customer_id,
                $date,
  ...
  // cleanup of a specific data type  
    ...
    //error_log( print_r(  'Function begin - vtprd_del_transient_cart_data_by_data_type, $transient_data_type= ' .$transient_data_type. '  $cart_id=' .$cart_id , true ) );
      ...
      $wpdb->query(
			$wpdb->prepare(
                "DELETE FROM `".VTPRD_TRANSIENT_CART_DATA."` WHERE  `cart_id` = %s  AND   `transient_data_type` = %s ",
                $cart_id, 
                $transient_data_type
			)
		);
  ...
  //general cleanup for a cart  
    ...
    //error_log( print_r(  'Function begin - vtprd_del_transient_cart_data_by_cart_id  $cart_id= ' .$cart_id, true ) );
  ...
  //************************* 
	function vtprd_destroy_session_save_keys() {
      ...
      if ( isset($_SESSION['vtprd_unique_cart_id']) ) { 
        $vtprd_unique_cart_id = sanitize_key($_SESSION['vtprd_unique_cart_id']);
      } else {
      ...
      if ( is_user_logged_in() &&  
         (isset($_SESSION['vtprd_customer_id']) ) )  {
        $vtprd_customer_id = sanitize_text_field($_SESSION['vtprd_customer_id']);
      } else {
        $vtprd_customer_id = false;
      }     


      //--==>  can't use this - it causes an endless loop!!!!!!!!!!!
      //v2.0.3 begin - save  "vtprd_orig_cart_price..."  session variables
      $saved_session_variables_array = array();
      foreach($_SESSION as $key => $value) {
         if (strpos($key,'vtprd_orig_cart_price') !== false) {
             $saved_session_variables_array[$key] = $value;
         }
      }      
      //v2.0.3 end

            
      //error_log( print_r(  'session_destroy 003 executed ', true ) );
      ...
      //--==>  can't use this - it causes an endless loop!!!!!!!!!!!
      //v2.0.3 begin - save  "vtprd_orig_cart_price..."  session variables
      if (sizeof($saved_session_variables_array > 0)) {
         foreach($saved_session_variables_array as $key => $value) {
             $_SESSION[$key] = $value;
         }
      }
      //v2.0.3 end
  ...
  // resets the unique session as well
	function vtprd_set_random_unique_cart_id($product_id=null) {
      ...
      //v2.0.3 begin - must remove vert pipes from key, will not pass sanitize_key function used later
      //$vtprd_unique_cart_id = time() .'||'. $product_id .'||'. $random_number;
      $vtprd_unique_cart_id = time() . $product_id . $random_number;
      //v2.0.3 end      
      
      $_SESSION['vtprd_unique_cart_id'] =  sanitize_key($vtprd_unique_cart_id);  //v2.0.3 sanitize the generated key at inception, as it is sanitized everywhere!!
  ...
  //************************* 
	function vtprd_unset_random_unique_cart_id() {
      $contents = sanitize_key($_SESSION['vtprd_unique_cart_id']);
  ...
  //************************* 
	function vtprd_find_or_set_unique_cart_id() {
        ...
        if (!isset($_SESSION['vtprd_unique_cart_id']) ) {           
            ...
            if ( is_user_logged_in() ) {
               $_SESSION['vtprd_customer_id'] = get_current_user_id();
               $cart_id = vtprd_get_transient_cart_id_by_customer_id ( sanitize_text_field($_SESSION['vtprd_customer_id']), 'customer_id_for_cart_id' );
               if ($cart_id) { 
   ...
   function vtprd_maybe_roll_out_prev_auto_insert_from_woo_cart($previous_auto_add_array, $all_or_single, $single_key=none) {      
      ...
      */
      ...
      $woocommerce_cart_contents = WC()->cart->get_cart(); //v2.1.0 $woocommerce->cart->get_cart(); 
      foreach($woocommerce_cart_contents as $key => $cart_item) {
        ...
        if ($cart_updated) {
          ...
          $_SESSION['internal_call_for_calculate_totals'] = true;   
            ...
            is LEGACY code, and the support may be taken away with any new version.
            The below code, I believe, will be needed.
            */
            //********************************************
     		 /*
         ...
         if ( ($woocommerce->cart->cart_contents_total == 0) &&
           ...
           //error_log( print_r(  'del woo_cart_contents_with_auto_add - 001 ', true ) ); 
  ...
  function  vtprd_get_admin_site_ip_address() {
    ...
    */
    ...
    if ( version_compare( $php_version, '5.3.1', '<' ) ) {
      //$vtprd_ip_address = $_SERVER['SERVER_ADDR']; //v2.0.3
      $vtprd_ip_address = vtprd_sanitize_text_or_array_field($_SERVER["SERVER_ADDR"]); //v2.0.3
    } else {    
      $host = vtprd_sanitize_text_or_array_field(gethostname() ); //v2.0.3
      $query = `nslookup -timeout=$timeout -retry=1 $host`;
      if(preg_match('/\nAddress: (.*)\n/', $query, $matches)) {
        $vtprd_ip_address =  trim($matches[1]);
      } else {
        $vtprd_ip_address = vtprd_sanitize_text_or_array_field(gethostbyname($host) ); //v2.0.3
      } 
    }	
  ...
  //v1.1.5 END   
   ...
   //V2.0.3  New Function
   //  USAGE:
   //  $allowed_html = vtprd_get_allowed_html();
   //**********************************     
  function  vtprd_get_allowed_html() { 
    //v2.0.3 begin  -  allowed_html used in "echo wp_kses" statements (which replace straight echo statements), for use with inline styles and other uses
   ...
   //V2.0.3  New Function
   //  USAGE:
   //  sanitize input arrays/fields, used from vtprd-rules-update.php
   // from https://wordpress.stackexchange.com/questions/24736/wordpress-sanitize-array
   //**********************************     
  function vtprd_sanitize_text_or_array_field($array_or_string) {
    ...
    elseif( is_array($array_or_string) ){
        foreach ( $array_or_string as $key => &$value ) {
            if ( is_array( $value ) ) {
                $value = vtprd_sanitize_text_or_array_field($value);
            }
            else {
   ...
   //V2.0.3  New Function
   //  USAGE:
   //  escape output arrays/fields, used from vtprd-rules-ui.php
   // from https://wordpress.stackexchange.com/questions/24736/wordpress-sanitize-array
   //**********************************     
  function  vtprd_escape_text_or_array_field($array_or_string) {
    $allowed_html = vtprd_get_allowed_html(); //v2.0.3 
    if( is_string($array_or_string) ){
        $array_or_string = wp_kses($array_or_string ,$allowed_html); 
    }
    elseif( is_array($array_or_string) ){
        foreach ( $array_or_string as $key => &$value ) {
            if ( is_array( $value ) ) {
                $value = vtprd_escape_text_or_array_field($value); 
            }
            else {
  ...
  //*************************
  //v2.1.0 New Function   (PHP 8.0 was kicking this out as a non-array)
  // ALWAYS Execute as::  $vtprd_rules_set = vtprd_get_rules_set();
  //*************************   
    function vtprd_get_rules_set() {  
        //global $vtprd_rules_set; - don't use global, array is returned
        
        $vtprd_rules_set_array   = get_option( 'vtprd_rules_set' );  
        
        If ($vtprd_rules_set_array) {
        	$vtprd_rules_set   = maybe_unserialize($vtprd_rules_set_array);  //maybe_unserialize is a WP function, performs the unserialize as needed
        } else {
            $vtprd_rules_set   = array();
        }
        
        return $vtprd_rules_set; 
   }   

  //*************************
  //v2.1.0 New Function   (PHP 8.0 was kicking this out as a non-array)
  // ALWAYS Execute as::  vtprd_set_rules_set($vtprd_rules_set);
  //*************************   
    function vtprd_set_rules_set($vtprd_rules_set) {  
        //global $vtprd_rules_set; - don't use global, array is always passed
        
        $vtprd_rules_set_array   = serialize($vtprd_rules_set);  
        update_option( 'vtprd_rules_set', $vtprd_rules_set_array );    //update_option also ADDS the option, if it's not there

        return; 
   }
   
   //**************************************
   //v2.1.0  moved here from parent-cart-validation
  //**************************************
  function vtprd_get_taxation_price_cart($product_id, $price) {
  ...
  function  vtprd_full_pro_upd_msg() {
      ...
      //(3)
      $homeURL1 = VTPRD_ADMIN_URL.'plugins.php';
      $homeURL2 = vtprd_strip_out_http($homeURL1); 
      $homeNAME = str_replace( '/wp-admin/plugins.php', '', $homeURL2  ?? '' );      //v2.0.3            
      
      
      
      $homeURL = VTPRD_ADMIN_URL.'plugin-install.php';
  ...
  //*************************
  //from http://stackoverflow.com/questions/15699101/get-client-ip-address-using-php
  function  vtprd_strip_out_http($url) {

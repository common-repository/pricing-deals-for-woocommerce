...
//EDITED * + * +  * + * +  * + * +  * + * + 
       //********
       //v2.0.2.02 all START AND CLOSE Sessions REMOVED 
       //********
...
class VTPRD_Apply_Rules{
	...
	public function __construct(){
      ...
      //error_log( print_r(  'REQUEST_URI= ' .$_SERVER["REQUEST_URI"], true ) );    
     ...
     if (strpos($pageURL,'plupload') !== false) {
       if ( $vtprd_setup_options['debugging_mode_on'] == 'yes' ){ 
          error_log( print_r(  'VTPRD_Apply_Rules INVALID, REQUEST_URI found = plupload, which shows up at trash/delete in wp-admin', true ) );
       }                                                                      
        return;     
     }
     //v2.1.0 end  
     ...
     if ( ($vtprd_info['current_processing_request'] == 'cart') 
            &&
         //((strpos($_SERVER["REQUEST_URI"],'wp-admin') !== false) ||  //v2.0.3
         ((strpos($pageURL,'wp-admin') !== false) ||  //v2.0.3
          (defined( 'DOING_CRON' ))) ) {           
    ...
    if ( $vtprd_setup_options['debugging_mode_on'] == 'yes' ){   
      ...
      if ($vtprd_info['current_processing_request'] == 'cart') { //v2.0.0
        global $woocommerce;
        //$woocommerce_cart_contents = $woocommerce->cart->get_cart(); //v2.1.0 
        $woocommerce_cart_contents = WC()->cart->get_cart(); //v2.1.0       
        $wooSize = is_array($woocommerce_cart_contents) ? sizeof($woocommerce_cart_contents) : 0;  //v2.1.0
    ...
    if ( $vtprd_setup_options['debugging_mode_on'] == 'yes' ) { 
      ...
      error_log( print_r(  '$_SESSION at APPLY-RULES BEGIN', true ) );
      $message = vtprd_sanitize_text_or_array_field($_SESSION);
    ...
    //v2.1.0 begin - moved down to the other side of "if ($vtprd_info['current_processing_request'] == 'cart') {"
    // as it already is coded on the " =='cart' " side
    /*
    //v2.0.1.0 begin
    ...
    */    
    ...
    if ($vtprd_info['current_processing_request'] == 'cart') {
       ...
       //Move parent cart contents to vtprd_cart 
         //error_log( print_r(  'vtprd_load_vtprd_cart_for_processing - CART rule', true ) );
        vtprd_load_vtprd_cart_for_processing(); 
  
       //v2.1.0  begin
        if ( $vtprd_setup_options['debugging_mode_on'] == 'yes' ) { 
          error_log( print_r(  '$vtprd_cart at APPLY-RULES BEGIN after Initial vtprd_cart Load', true ) ); 
       ...
       //v2.0.1.0 end 
        ...
        $woocommerce_cart_contents = WC()->cart->get_cart(); //v2.1.0   
        $wooSize = is_array($woocommerce_cart_contents) ? sizeof($woocommerce_cart_contents) : 0;  //v2.1.0
        $vtprdSize = is_array($vtprd_cart->cart_items) ? sizeof($vtprd_cart->cart_items) : 0;  //v2.1.0
    ...
    //CATALOG RULES ****
      ...
      //v2.1.0 begin - moved down here from above
      // after getting rules set, update rule status for those rules which don't meet customer-level criteria
       //$sizeof_rules_set = sizeof($vtprd_rules_set);  //v2.1.0
      $sizeof_rules_set = is_array($vtprd_rules_set) ? sizeof($vtprd_rules_set) : 0; //v2.1.0        
      for($i=0; $i < $sizeof_rules_set; $i++) { 
       ...
       if ( ($vtprd_info['current_processing_request'] == 'cart') && 
           ...
           switch( $vtprd_setup_options['limit_cart_discounts'] ) {
            ...
            case 'allow_no_standalone_woo_coupon_discounts_if_coupon_actuated_pricing_deal_granted':  //6 
                if (sanitize_text_field($_SESSION['coupon_activated_discount'])) { //coupon_activated_discount = PREVIOUS rule was coupon activated and discounted
                   vtprd_remove_any_other_woo_coupons();  
                } 
               break;              
           }
      ...
      if ($vtprd_info['current_processing_request'] == 'cart') {
        ...
        if ($vtprdSize < $wooSize)  { //can be =, which is standard, and if vtprd is greater, there's an auto-add in progress        
          ...
          //RELOAD vtprd_cart in this situation - it'll be because Catalog rules were just processed...
           //error_log( print_r(  'vtprd_load_vtprd_cart_for_processing - CART rule RELOAD', true ) );
          vtprd_load_vtprd_cart_for_processing(); 
 ...
 exceeds the product's cumulative quantity discount.
  ...
  public function vtprd_upd_cart_discount($i, $d, $curr_prod_array) {   
    ...
    $cart_id = sanitize_key($_SESSION['vtprd_unique_cart_id']); //v2.0.2.0
 ...
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 022 ', true ) );
    ...
    public function vtprd_are_brands_in_list_check($i,$k, $incl_or_excl, $test_this_array) {
      ...
      if (sizeof($vtprd_cart->cart_items[$k]->brands_list) == 0) {
        ...
        $vtprd_cart->cart_items[$k]->brands_tax_found =  wp_kses($vtprd_cart->cart_items[$k]->brands_tax_found ,$allowed_html); //v2.0.3 
    ...
    public function vtprd_are_groups_in_inPop_list_check($i,$k, $incl_or_excl, $test_this_array) {
       ...
       */              
       ...
       foreach ($test_this_array as $group_ID) {
         ...
         $name = $wpdb->get_var( $wpdb->prepare( "SELECT `name` FROM `" . $wpdb->prefix . "groups_group` WHERE `group_id`= %s LIMIT 1",$group_ID ) );
         //v2.0.3 end
  ...
  //***********************************************************     
   public  function vtprd_roll_free_products_out_of_other_rules($i) {
    ...
    //$sizeof_ruleset = sizeof($vtprd_rules_set); //v2.1.0
    $sizeof_rules_set = is_array($vtprd_rules_set) ? sizeof($vtprd_rules_set) : 0; //v2.1.0
    ...
    foreach($vtprd_rules_set[$i]->free_product_array as $free_product_key => $free_qty) {  
      ...
      for($rule=0; $rule < $sizeof_rules_set; $rule++) {   //v2.1.0

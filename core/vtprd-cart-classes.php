...
class VTPRD_Cart {	
 ...
 //   CART-LEVEL UNITS AND AMOUNTS AS RELATES TO RULES ITERATIONS, INPUT AND OUTPUT..  mAYBE PUT THE NEW CART-ITEM STUFF HERE....
    ...
    public $save_transient_cart_id = '';  //v2.0.3 - the session cart_id disappears in post purchase thankyou, so stored here.    
    ...
    $this->save_transient_cart_id;  //v2.0.3 - the session cart_id disappears in post purchase thankyou, so stored here.                       
  }
...
class VTPRD_Cart_Functions{
    ...
    */
    public function vtprd_cart_oldprice() { 
        ...
        echo wp_kses('$vtprd_cart->$cart_oldprice' ,$allowed_html ); //v2.0.3
    }
    ...
    */    
    public function vtprd_cart_yousave() { 
        ...
        $allowed_html = vtprd_get_allowed_html(); //v2.0.3
        echo wp_kses('$vtprd_cart->$cart_yousave' ,$allowed_html ); //v2.0.3       
    }
    ...
    public function vtprd_cart_unit_oldprice($product_id) { 
        ...
        foreach($vtprd_cart->vtprd_cart_items as $key => $vtprd_cart_item) {
           if ($vtprd_cart_item->product_id == $product_id) {
              $allowed_html = vtprd_get_allowed_html(); //v2.0.3
              echo wp_kses($vtprd_cart->cart_unit_oldprice ,$allowed_html ); //v2.0.3 
              break;
           }
        }
    }
    ...
    */    
    public function vtprd_cart_total_oldprice($product_id) { 
        ...
        foreach($vtprd_cart->vtprd_cart_items as $key => $vtprd_cart_item) {
           if ($vtprd_cart_item->product_id == $product_id) {
              $allowed_html = vtprd_get_allowed_html(); //v2.0.3
              echo wp_kses($vtprd_cart_item->cart_total_oldprice ,$allowed_html ); //v2.0.3               
              break;
           }
        }
    }
    ...
    */    
    public function vtprd_cart_total_yousave($product_id) { 
        ...
        foreach($vtprd_cart->vtprd_cart_items as $key => $vtprd_cart_item) {
           if ($vtprd_cart_item->product_id == $product_id) {
              $allowed_html = vtprd_get_allowed_html(); //v2.0.3
              echo wp_kses($vtprd_cart->cart_total_yousave ,$allowed_html ); //v2.0.3 
              break;
           }
        }
    }    

...
function vtprd_pro_upgrade_cntl() {
    ?>
		<div id="icon-themes" class="icon32"></div> 
              ...
              <a  href=" <?php echo esc_url(VTPRD_PURCHASE_PRO_VERSION_BY_PARENT) ; ?> "  title="Purchase Pro">
                  <img id="pricing-deals-img" alt="help" height="40px" width="40px" src="<?php echo esc_url(VTPRD_URL.'/admin/images/sale-circle.png');?>" />
              </a>
              <h2><?php esc_attr_e('Upgrade to Pricing Deals Pro', 'vtprd'); ?></h2>
            </div>  
            <h2 id="upgrade-blurb" ><?php esc_attr_e('Pricing Deals Pro', 'vtprd'); ?> </h2>
            <span class="buy-button-area">
              <a href="<?php echo esc_url(VTPRD_PURCHASE_PRO_VERSION_BY_PARENT); ?>" class="buy-button">
    ...
    <div id="upgrade-div">       
      ...
      <ul id="vtprd-main-attributes">
        <li> <span class="vtprd-highlight"><?php esc_attr_e('Group Power &nbsp;-&nbsp; Apply rules to <em>any group you can think of!</em>', 'vtprd') ?></span>
          <ul> <strong><em><?php esc_attr_e('Filter By', 'vtprd') ?></em></strong>
            <li><?php esc_attr_e(' -  Wholesale / Membership / Role (Logged-in Status)', 'vtprd') ?></li>
            <li><?php esc_attr_e(' -  Product Category', 'vtprd') ?></li>
            <li><?php esc_attr_e(' -  Pricing Deal Custom Category', 'vtprd') ?></li>
            <li><?php esc_attr_e(' -  Product', 'vtprd') ?></li>
            <li><?php esc_attr_e(' -  Variations', 'vtprd') ?></li>
          </ul>             
        </li>
        <li><span class="vtprd-highlight"><?php esc_attr_e('Product-level Deal Exclusion', 'vtprd') ?></span></li>
        <li><span class="vtprd-highlight"><?php esc_attr_e('Maximum Deal Limits, including "One Per Customer" limit', 'vtprd') ?></span></li>
      </ul>
      
      <ul>  
        <li><?php esc_attr_e('<em>Deal Types Now have Tremendous Additional Power, with full filtering capability</em>, including:', 'vtprd') ?>
          <ul>
            <li><?php esc_attr_e('BOGO (Buy One, Get One) [for All]', 'vtprd') ?></li>
            <li><?php esc_attr_e('Sale Pricing [for All]', 'vtprd') ?></li>
            <li><?php esc_attr_e('Group Pricing [for All]', 'vtprd') ?></li>
            <li><?php esc_attr_e('Special Promotions [for All]', 'vtprd') ?></li>
          </ul>         
        </li>

        <li><?php esc_attr_e('Using Pricing Deal Custom Categories makes Group pricing and many other Discount Types *much more powerful*', 'vtprd') ?>
          <ul>
            <li><?php esc_attr_e('Group together any products you elect seamlessly', 'vtprd') ?></li>
            <li><?php esc_attr_e('Special Price for this Group', 'vtprd') ?></li>
            <li><?php esc_attr_e('Grouping affects no other part of your store', 'vtprd') ?></li>
            <li><?php esc_attr_e('Pricing Deal Custom Categories *do not affect* Product Category store organization and presentation *in any way*', 'vtprd') ?></li>
          </ul>
        </li>
      </ul>
      <span class="buy-button-area">                                 
 ...
 <?php
  ...
  //***************************
  //v2.0.3 RECODED to do all updates LIVE without user interaction, IN vt-pricing-deals.php.
  //***************************  
function vtprd_show_help_page_cntl() {
       ...
       $doThis = sanitize_text_field( $_GET["doThis"] );   //v2.0.3 
       //************************
       ...
       $url = esc_url("https://www.varktech.com/documentation/pricing-deals/introrule/");          
        ?>
                   
        <div class="wrap">
      		<div id="icon-themes" class="icon32"></div>        
      		    <a class="pricing-deal-button" target="_blank" href="<?php echo esc_url($url);?>"><?php esc_attr_e('Open Documentation Page!', 'vtprd');?></a>          
            ...
            <?php  if(defined('VTPRD_PRO_DIRNAME')) { ?>
              <h2><?php esc_attr_e('Pricing Deals Pro', 'vtprd'); ?></h2>
            <?php } else { ?>
              <h2><?php esc_attr_e('Pricing Deals', 'vtprd'); ?></h2>
            <?php } ?>
            <?php //v2.0.0 end ?>
            <h2>
       ...
       //error_log( var_export($doThis_array, true ) );     
        ...
        foreach ($doThis_array as $doThis)  {       
            switch ($doThis) {
              ...
              //******************************************************************             
               case  'showRuleInfo' :         
                    $rule_id = sanitize_text_field( $_GET["ruleID"] );  //v2.0.3 added esc_attr( )

                    
                    $message = $this->vtprd_return_rule_info($rule_id);                 
                     ?> 
                       ...
                       //***************************
                       //v2.0.3
                       tried to use esc_js on the onclick event, but it would not let the code through.
                       Reviewing the ***poorly presented*** documentation, 
                       it seems that esc_js is only supposed to
                       be used when supplying variable data to the js.           
            
                       
           
                       // esc_js('this.focus(); this.select();')     DOES NOT WORK
                       
                       readonly="readonly"  onclick="this.focus(); this.select();"  id="show-rule-textarea" 
                       ONCLICK REPLACED WITH INLINE JS initiated from vt-pricing-deals.php
                        ...
                        <div id="show-rule-area"> 
                           <textarea 
                              style="
                                ...
                                display: block;
                              "                                                       
                           readonly="readonly"  class= "textarea-focus"  id="show-rule-textarea" 
                           name="system-rule-textarea" title="To copy the RULE info to send to SUPPORT, click below then press Ctrl + C (PC) or Cmd + C (Mac)."><?php echo wp_kses($message ,$allowed_html ); ?></textarea>
 ...
 function vtprd_v2020_data_conversion($forceDataUpd=null){ 
  //convert rule data to use new structures 'buy_group_framework', 'action_group_framework'   
      ...
      //create backup before updating ruleset
        ...
        //v2.1.0 begin - PHP 8.0 was kicking this out as a non-array
        // HOWEVER, this occurs in a version PRIOR to v2.1.0, so is contraindicated
  ...
  function vtprd_report_v2020_conversion() { 
      ...
      $allowed_html = vtprd_get_allowed_html(); //v2.0.3
      $message  = '<h1 style="clear:left;float:left;padding-top:50px;color:grey;">V2.0.2.0 &nbsp;&nbsp; Data Conversion upgraded the internal structure of the rules Include/Exclude functions.</h1>';
      $message .= '<h1 style="clear:left;float:left;padding-top:20px;padding-left: 9%;color:grey;">Rule Include/Exclude Detail Summaries &nbsp; completed Successfully!</h1>';
      ...
      if ( (!is_array($vtprd_v2020_conversion_report)) 
        ...
        $message = '<h1 style="clear:left;float:left;margin-top:50px;">No Rule Conversion was necessary for Version 2.0.2.0</h1>'; 
        echo wp_kses($message ,$allowed_html); //v2.0.3
        return;     
      }
      ...
      foreach ($vtprd_v2020_conversion_report as $conversion_report) {
        ...
        //     ($conversion_report['action_group_products']) ) {
          ...
          $message = '<br><br><br><h3 style="clear:left;float:left;color:green;">RULE: ' .$conversion_report['rule_title']. '&nbsp;&nbsp;&nbsp;&nbsp; Rule ID: ' .$conversion_report['rule_id']. '&nbsp;&nbsp;&nbsp;&nbsp; Rule Type: ' .$rule_type. '</h3>';
          echo wp_kses($message ,$allowed_html); //v2.0.3
  ...
  function vtprd_print_v2020_report_line($field_name,$conversion_value) {   
      ...
      switch( $field_name ) {      
          case 'buy_group_products': 
              $message = '<h4 style="clear:left;float:left;color:green;margin-top: -5px;"><span style="font-style: normal;font-size: 18px;"> Qualify Group Products Area &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span> ';
              echo wp_kses($message ,$allowed_html ); //v2.0.3
              break;                      
          case 'buy_group_customer':
              $message = '<h4 style="clear:left;float:left;color:green;margin-top: -5px;"><span style="font-style: normal;font-size: 18px;"> Qualify Group Customer Area &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span> ';
              echo wp_kses($message ,$allowed_html ); //v2.0.3
              break;            
          case 'action_group_products':          
              $message = '<h4 style="clear:left;float:left;color:green;margin-top: -5px;"><span style="font-style: normal;font-size: 18px;"> Discount Group Products Area &nbsp;&nbsp;&nbsp;&nbsp; </span> ';
              echo wp_kses($message ,$allowed_html ); //v2.0.3
              break; 
            break;  
      }
      ...
      switch( $conversion_value ) {      
          case 'both': 
              $message = 'Population has been Set to BOTH INclude and EXclude &nbsp;&nbsp;&nbsp; <span style="color:grey !important;">(both include and exclude paramaters are active in the Group)</span> </h4>';
              echo wp_kses($message ,$allowed_html ); //v2.0.3
              break;                      
          case 'includeOnly':
              $message = 'Population has been Set to INclude &nbsp;&nbsp;&nbsp; <span style="color:grey !important;">((only include paramaters are active in the Group)</span> </h4>';
              echo wp_kses($message ,$allowed_html ); //v2.0.3
              break;            
          case 'excludeOnly':          
              $message = 'Population has been Set to EXclude &nbsp;&nbsp;&nbsp; <span style="color:grey !important;">(only exclude paramaters are active in the Group)</span> </h4>';
              echo wp_kses($message ,$allowed_html ); //v2.0.3
              break; 
            break;  
          case 'none':
          case '':           
              $message = 'Population has NO Include or Exclude</h4>';
              echo wp_kses($message ,$allowed_html ); //v2.0.3
              break; 
            break; 
          default:           
              $message = 'Population set to '.$conversion_value.'.  Population has no Include or Exclude </h4>';
              echo wp_kses($message ,$allowed_html ); //v2.0.3
              break; 
            break;                        
      }      
  ...
  function vtprd_print_v2020_nothing_done_line($buyORaction, $rule_type, $group_type) {   
      ...
      if ($buyORaction == 'buy') {
        ...
        $message  = '<h4 style="clear:left;float:left;color:green;margin-top: -5px;"><span style="font-style: normal;font-size: 18px;"> Qualify Group Already set to &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Any Product </span> ';
        $message .= '&nbsp;&nbsp;&nbsp; <span style="color:grey !important;">Nothing Done</span> </h4>';
        echo wp_kses($message ,$allowed_html ); //v2.0.3
        
      } else {
        if ($rule_type == 'cart') {
          $message = '<h4 style="clear:left;float:left;color:green;margin-top: -5px;"><span style="font-style: normal;font-size: 18px;"> Discount Group Already set to </span> ';
          echo wp_kses($message ,$allowed_html ); //v2.0.3
          
          switch( $group_type ) { 
              case 'sameAsInPop': 
                  $message = '&nbsp;&nbsp;&nbsp;&nbsp; Discount Group Same as Buy Group &nbsp;&nbsp;&nbsp; </span> ';
                  echo wp_kses($message ,$allowed_html ); //v2.0.3
                  break;                      
              case 'wholeStore':
                  $message = '&nbsp;&nbsp;&nbsp;&nbsp; Any Product &nbsp;&nbsp;&nbsp; </span> ';
                  echo wp_kses($message ,$allowed_html ); //v2.0.3
                  break;                       
          }
   ...
   function vtprd_v2020_alter_table() { 
      ...
      $is_this_table_here= $wpdb->get_var(
    		$wpdb->prepare(
                  "SHOW TABLES LIKE %s ",
                  $lifetime_table_name               
    		)
      ...
      */ 
      ...
      $varsql = $wpdb->prepare ( "DESC {%s}", $lifetime_table_name );
      $existing_columns_array = $wpdb->get_col ( $varsql, 0);
      //v2.0.3 end
	  ...
	  $wpdb->query( $wpdb->prepare ( "ALTER TABLE %s  ADD `purchaser_user_id` bigint(20) ", $lifetime_table_name) );  //v2.0.3  %1s prevents prepare from adding single quotes
      ...
      $sql = $wpdb->prepare (  "SELECT * FROM %s ", $lifetime_table_name);  //v2.0.3  %1s prevents prepare from adding single quotes
      $vtprd_purchase_log = $wpdb->get_results($sql,ARRAY_A) ;
      //v2.0.3 end
      ...
      if ($vtprd_purchase_log) {
        ...
        for($i=0; $i < $sizeof_purchase_log; $i++) { 
          ...
          if ($row['purchaser_email']) {
             ...
             $user_id 	= $wpdb->get_var( $wpdb->prepare ( "SELECT ID  FROM %s  WHERE `user_email` = %s ",
                            $user_table_name,
                            $user_email 
                            )
                        ); 
          ...
          //error_log( print_r(  '$purchaser_table_id = ' .$purchaser_table_id , true ) );
                ...
                $wpdb->query( $wpdb->prepare(  "UPDATE %s  SET `purchaser_user_id` = %s WHERE `purchaser_table_id` = %s ", $lifetime_table_name, $user_id, $purchaser_table_id ) );   //v2.0.3
                //v2.0.3  %1s prevents prepare from adding single quotes
 ...
 function vtprd_v2point0_data_conversion($forceDataUpd=null){ 
  //convert rule data to use new structures 'buy_group_framework', 'action_group_framework'   
      ...
      $ruleset_contains_auto_add_free_coupon_initiated_deal = 'no';
       //$sizeof_rules_set = sizeof($vtprd_rules_set);  //v2.1.0
      $sizeof_rules_set = is_array($vtprd_rules_set) ? sizeof($vtprd_rules_set) : 0; //v2.1.0  - KEEP THIS
      for($i=0; $i < $sizeof_rules_set; $i++) { 
...
*/      
      ...
      global $wpdb;          
      $contentSearch = 'product'; //v2.0.3 - $wpdb->prepare REQUIRES a variable in the sql.
      $varsql = $wpdb->prepare( "SELECT posts.`ID`
            			FROM `".$wpdb->posts."` AS posts			
            			WHERE posts.`post_status` = 'publish' AND posts.`post_type`= %s ORDER BY posts.`ID` ASC",$contentSearch );  //v2.0.3                  
  ...
  function vtprd_return_rule_info($rule_id) { 
        ...
        //v2.0.0.8 begin
        $bulk_deal_array_count = sizeof($vtprd_rules_set[$i]->bulk_deal_array);   //v2.1.0 replace $vtprd_rule-> with  $vtprd_rules_set[$i]->
        if ($bulk_deal_array_count > 0) { 
          $send_data .= ' BULK DEAL        '  . "\n"; 
          $send_data .= var_export($vtprd_rules_set[$i]->bulk_deal_array, true );   //v2.1.0 replace $vtprd_rule-> with  $vtprd_rules_set[$i]->    
  ...
  function vtprd_report_outmoded_incl_excl_products() { 
      ...
      $message = '<h1 style="clear:left;float:left;padding-top:50px;color:grey;">Version 2.0 - General Conversion completed Successfully!</h1>';
      echo wp_kses($message ,$allowed_html ); //v2.0.3
      
      if (!defined('VTPRD_PRO_VERSION')) {

        $message = '<h1 style="clear:left;float:left;padding-top:50px;color:grey;">ALL RULES reported on below have includes/excludes. They will ONLY work when the <br><br>PRO Version 2.0.0 <br><br>is <em>installed</em> and <em>active</em> . </h1>';
        echo wp_kses($message ,$allowed_html ); //v2.0.3
      }
      ...
      if (($vtprd_v2_conversion_report) &&
        ...
        $message = '<h1 style="clear:left;float:left;margin-top:50px;">No Product Include/Exclude Conversion was necessary for Version 2.0</h1>'; 
        echo wp_kses($message ,$allowed_html ); //v2.0.3
        return;     
      }
        ...
        if ($conversion_report['details']['includeOrExclude_option'] != 'includeAll') {
          ...
          ?>
            ...
            <div class="includeOrExclude_area">
              <?php  $homeURL = VTPRD_ADMIN_URL.'post.php?post= ' .wp_kses($prod_id ,$allowed_html). ' &action=edit';      ?>
              <h4 class="includeOrExclude-area-title"><?php echo wp_kses('Product ' ,$allowed_html) .$prod_id. ' - &nbsp;&nbsp;<a class="runDataUpd" target="_blank" href="'.esc_url($homeURL).'">' .wp_kses($product->post_title ,$allowed_html). '</a>' .wp_kses($product_msg ,$allowed_html); ?></h4>                    
              <div class="dropdown includeOrExclude_areaID clear-left">              
                 <span class="dropdown-label includeOrExclude_label"><?php esc_attr_e(' Original Product Include/Exclude Selection: ', 'vtprd');?>
                  <?php
                  ...
                  ?>            
                    <span class="includeOrExclude_action"><?php echo wp_kses($action ,$allowed_html); ?></span>
                 </span>                               
              </div>
  ...
  function vtprd_print_outmoded_report($outmoded_incl_excl_products_report) {
   ...
   function vtprd_v2point0_create_tables(){ 
      ...
      if ( (isset ($vtprd_data_update_options['optional_updates'])) &&
        ...
        echo esc_attr_e('Table Update already done!' , 'vtprd'); 
        return;
      } 
      ...
      echo esc_attr_e('Table Update Complete!' , 'vtprd');
      return;
   }  
   ...
   function vtprd_v2point0_alter_table_column(){ 
      ...
      global $wpdb;
      $altTab = 'ALTER TABLE';  //v2.0.3 - $wpdb->prepare REQUIRES a variable in the sql.  
	  $wpdb->query( $wpdb->prepare( "%1s `".VTPRD_PURCHASE_LOG."`  CHANGE `ruleset_object` `ruleset_object` LONGTEXT '';", $altTab  )); //v2.0.3  %1s prevents prepare from adding single quotes
      $wpdb->query( $wpdb->prepare( "%1s `".VTPRD_PURCHASE_LOG."`  CHANGE `cart_object` `cart_object` LONGTEXT '';", $altTab  )); //v2.0.3  %1s prevents prepare from adding single quotes
...
function vtprd_setup_options_cntl() {
  ...
  //$vtprd_backbone->vtprd_add_help_tab ();
    ...
    if ($message) {
      ...
      $message = '<div class="fade is-dismissible vtprd-run-data-upd" 
        style="
              line-height: 19px;
              padding: 11px 15px;
              text-align: left;
...
?>        
	...
	<div class="wrap">                                                                        
      ...
      ?>    
		...
		<form method="post" action="options.php">
          ...
          //WP functions to execute the registered settings!
                    ...
                    $url = esc_url("https://www.varktech.com/documentation/pricing-deals/settings");
			?>		
      ...
      <div id="floating-buttons" class="show-buttons">
        ...
        <a class="button-secondary" target="_blank" href="<?php echo esc_url($url);?>"><?php esc_attr_e('Help!', 'vtprd');?></a> 
      </div>
      ...
      <span id="vtprd-system-buttons-anchor"></span>  
       <p id="system-buttons">
          ...
          <a  id="nuke-cart-button"  class="nuke_buttons button-second" href="<?php echo esc_url(VTPRD_ADMIN_URL.'edit.php?post_type=vtprd-rule&page=vtprd_license_options_page&action=force_plugin_updates_check');?>">Plugin Updates Check</a> <?php //v1.1.8.2 removed home_url ?>
    ...
    <span class="system-info">
       <span class="system-info-line"><span class="system-info-label">FREE_VERSION: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_VERSION ,$allowed_html); ?></span> </span>
       <span class="system-info-line"><span class="system-info-label">FREE_LAST_UPDATE_DATE: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_LAST_UPDATE_DATE ,$allowed_html); ?></span></span>
       <span class="system-info-line"><span class="system-info-label">FREE_DIRNAME: </span> <span class="system-info-data"><?php echo esc_url(VTPRD_DIRNAME); ?></span></span>
       <span class="system-info-line"><span class="system-info-label">URL: </span> <span class="system-info-data"><?php echo esc_url(VTPRD_URL);  ?></span></span>
       <span class="system-info-line"><span class="system-info-label">EARLIEST_ALLOWED_WP_VERSION: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_EARLIEST_ALLOWED_WP_VERSION ,$allowed_html); ?></span></span>
       <span class="system-info-line"><span class="system-info-label">WP VERSION: </span> <span class="system-info-data"><?php echo wp_kses($wp_version ,$allowed_html);?></span> </span>
       <span class="system-info-line"><span class="system-info-label">EARLIEST_ALLOWED_PHP_VERSION: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_EARLIEST_ALLOWED_PHP_VERSION ,$allowed_html);?></span> </span>
       <span class="system-info-line"><span class="system-info-label">FREE_PLUGIN_SLUG: </span> <span class="system-info-data"><?php echo esc_url(VTPRD_PLUGIN_SLUG);  ?></span></span>
     </span> 
    
    <h4 class="system-info-subtitle">Parent Plugin Info</h4>
    <span class="system-info">
       <span class="system-info-line"><span class="system-info-label">PARENT_PLUGIN_NAME: </span> <span class="system-info-data"><?echo wp_kses(VTPRD_PARENT_PLUGIN_NAME ,$allowed_html); ?></span> </span>
       <span class="system-info-line"><span class="system-info-label">EARLIEST_ALLOWED_PARENT_VERSION: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_EARLIEST_ALLOWED_PARENT_VERSION ,$allowed_html); ?></span></span>

       <?php if(defined('WOOCOMMERCE_VERSION') && (VTPRD_PARENT_PLUGIN_NAME == 'WooCommerce')) { ?>
       <span class="system-info-line"><span class="system-info-label">PARENT_VERSION (WOOCOMMERCE): </span> <span class="system-info-data"><?php echo wp_kses(WOOCOMMERCE_VERSION ,$allowed_html); ?></span></span>
       <?php } ?>

       <span class="system-info-line"><span class="system-info-label">TESTED_UP_TO_PARENT_VERSION: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_TESTED_UP_TO_PARENT_VERSION ,$allowed_html); ?></span></span>
     ...
     <?php   if (defined('VTPRD_PRO_DIRNAME')) {  ?> 
      ...
      <span class="system-info">      
       <span class="system-info-line"><span class="system-info-label">PRO_PLUGIN_NAME: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_PRO_PLUGIN_NAME ,$allowed_html );?></span> </span>
       <span class="system-info-line"><span class="system-info-label">PRO_FREE_PLUGIN_NAME: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_PRO_FREE_PLUGIN_NAME ,$allowed_html );?></span> </span>
       <span class="system-info-line"><span class="system-info-label">PRO_VERSION: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_PRO_VERSION ,$allowed_html );?></span> </span>
       <span class="system-info-line"><span class="system-info-label">PRO_LAST_UPDATE_DATE: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_PRO_LAST_UPDATE_DATE ,$allowed_html ); ?></span></span>
       <span class="system-info-line"><span class="system-info-label">PRO_DIRNAME: </span> <span class="system-info-data"><?php echo esc_url(VTPRD_PRO_DIRNAME); ?></span></span>
       <span class="system-info-line"><span class="system-info-label">PRO_MINIMUM_REQUIRED_FREE_VERSION: </span> <span class="system-info-data"><?php echo wp_kses(VTPRD_PRO_MINIMUM_REQUIRED_FREE_VERSION ,$allowed_html ); ?></span></span>

       <span class="system-info-line"><span class="system-info-label">PRO_PLUGIN_SLUG: </span> <span class="system-info-data"><?php echo esc_url(VTPRD_PLUGIN_SLUG); ?></span> </span>
       <span class="system-info-line"><span class="system-info-label">PRO_REMOTE_VERSION_FILE: </span> <span class="system-info-data"><?php echo esc_url(VTPRD_PRO_REMOTE_VERSION_FILE); ?></span> </span>
      </span> 
     <?php   }  ?>   

        
     <?php   if ( $vtprd_setup_options['debugging_mode_on'] == 'yes' ){  
               $message = '<pre>'.print_r( $your_system_info , true).'</pre>'; //v2.0.3 ?>  
     <h4 class="system-info-subtitle">Debug Info</h4>
      <span class="system-info">                  
       <span class="system-info-line"><span class="system-info-label">PHP VERSION: </span> <span class="system-info-data"><?php echo wp_kses(phpversion() ,$allowed_html ); ?></span> </span>
       <span class="system-info-line"><span class="system-info-label">SYSTEM MEMORY: </span> <span class="system-info-data"><?php echo wp_kses($message ,$allowed_html );  ?></span> </span>
     </span> 
     <?php   }  ?>
...
function vtprd_processing_options_callback () {
    ...
    <?php //v2.0.0.2 begin 
      ...
      if ( version_compare( $wp_version, '4.8', '>=' ) ) {
        $url = esc_url("https://wordpress.org/plugins/classic-editor/");
    ?>
      <h4 style="color:black !important;">WordPress 5.0+ - "Product Wholesale Checkbox on Product Page" may be <strong>only available</strong> using the 
       <a target="_blank" href="<?php echo esc_url($url);?>"><?php esc_attr_e('Classic Editor Plugin', 'vtprd');?></a>
...
function vtprd_nav_callback() {                                      
    ...
    <img id="pricing-deals-img-preload" alt="" src="<?php echo esc_url(VTPRD_URL.'/admin/images/upgrade-bkgrnd-banner.jpg');?>" />
 		<div id="upgrade-title-area">
      <a  href=" <?php echo esc_url(VTPRD_PURCHASE_PRO_VERSION_BY_PARENT) ; ?> "  title="Purchase Pro">
      <img id="pricing-deals-img" alt="help" height="40px" width="40px" src="<?php echo esc_url(VTPRD_URL.'/admin/images/sale-circle.png');?>" />
      </a>
      <h2><?php esc_attr_e('Pricing Deals', 'vtprd'); ?>
          <?php
            if(defined('VTPRD_PRO_DIRNAME')) { 
      ...
      <?php if(!defined('VTPRD_PRO_DIRNAME')) {  ?> 
          <span class="group-power-msg"><strong><?php esc_attr_e('Get Group Power', 'vtprd'); ?></strong>
              &nbsp;-&nbsp; 
            <?php esc_attr_e('Apply rules to ', 'vtprd'); ?>
            <em>
            <?php esc_attr_e('any group you can think of, and More!', 'vtprd'); ?>
            </em>
          </span> 
          <span class="buy-button-area">
            <a href="<?php echo esc_url(VTPRD_PURCHASE_PRO_VERSION_BY_PARENT); ?>" class="buy-button">
                <span class="buy-button-label" style="width: 95%;"><?php esc_attr_e('Get Pricing Deals Pro', 'vtprd'); ?></span>
            </a>
          </span> 
      <?php }  ?>
    ...
    */
    ?>
         ...
         <div id="vtprd-options-menu">        
              <ul>                                                           
                ...
                <li class="discount_display_jumps">
                  <a href="#vtprd-checkout-reporting-anchor" title="Discount Checkout Display"><?php esc_attr_e('Checkout Display', 'vtprd'); ?></a>
                  <span class="discount_display_jumps">|</span>
                </li>  
                
                <li class="discount_display_jumps">
                  <a href="#vtprd-cartWidget-options-anchor" title="Discount Cart Widget Display"><?php esc_attr_e('Cart Widget Display', 'vtprd'); ?></a>
                  <span class="discount_display_jumps">|</span>
                </li>  
                
                <li>
                  <a href="#vtprd-catalog-options-anchor" title="Discount Catalog Display"><?php esc_attr_e('Price Display', 'vtprd'); ?></a>
                  <span>|</span>
                </li>  

                <li>
                  <a href="#vtprd-processing-options-anchor" title="Processing Options"><?php esc_attr_e('Processing Options', 'vtprd'); ?></a>
                  <span>|</span>
                </li>  

                <li>
                  <a href="#vtprd-lifetime-options-anchor" title="Lifetime Discount Options"><?php esc_attr_e('Customer Limit Options', 'vtprd'); ?></a>
                  <span>|</span>
                </li>  

                <li>
                  <a href="#vtprd-system-options-anchor" title="System and Debug Options"><?php esc_attr_e('System Options', 'vtprd'); ?></a>
                  <span>|</span>
                </li>  

                <li>
                  <a href="#vtprd-system-buttons-anchor" title="System Buttons"><?php esc_attr_e('System Buttons', 'vtprd'); ?></a>
                  <span>|</span>  <?php //v2.0.2.0 ?>
                </li>  
                 
                 <?php //v2.0.2.0 begin ?> 
                <li>
                  <a href="#vtprd-delete-plugin-buttons-anchor" title="Delete all files Button"><?php esc_attr_e('Set Override to Delete ALL on Uninstall', 'vtprd'); ?></a>
                </li> 
                <?php //v2.0.2.0 begin ?>               
      ...
      $allowed_html = vtprd_get_allowed_html(); //v2.0.3
      echo wp_kses('<br>',$allowed_html ); //v2.0.3 
    ...
    if ( $options['debugging_mode_on'] == 'yes' ){ 
          ...
          $message = '<div id="message" class="error fade is-dismissible" style="background-color: #FFEBE8 !important; width:80%; margin-left:10%;"><p>' . $message . ' </p></div>'; //v2.0.0.9 added width:80%; margin-left:10%;
          echo wp_kses($message ,$allowed_html ); //v2.0.3    
    }
    //v2.0.0 end
...
function vtprd_taken_where_options_callback () {
    ...
    <h4 id="vtprd-discount-messaging"><?php // esc_attr_e('Cart Discount Taken Where - in the Unit Price for each product or as a single Woo Coupon?', 'vtprd'); ?> 
          <strong><?php esc_attr_e('Cart Discount Display', 'vtprd'); ?></strong>
...
function vtprd_checkout_options_callback () {
    ...
    <h4 id="vtprd-checkout-reporting"><?php esc_attr_e('These options control Pricing Deal checkout display.', 'vtprd'); ?>
      <a id="help-all" class="help-anchor" href="<?php esc_js("javascript:void(0);");?>" >
      <?php //esc_attr_e('Show All:', 'vtprd'); v2.0.3 removed?> 
      &nbsp; <span> <?php //esc_attr_e('More Info', 'vtprd'); v2.0.3 removed?> </span></a>     
    </h4> 
    <?php 
}
...
function vtprd_show_old_price_callback () {   //opt35  documentation only, no switches set here!   
  $allowed_html = vtprd_get_allowed_html(); //v2.0.3
    ...
    ?>
     <span><?php esc_attr_e('When a Catalog Rule Discount rule is applied, the Old Price and You Save Messages can be displayed.', 'vtprd');?>
        <strong><em><?php esc_attr_e('See', 'vtprd');?></em> =></strong>
     <a id="help35" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
     </span>
    <?php
    $html = ''; 
  ...
  $html .= '</p>';
    ...
    echo wp_kses($html ,$allowed_html ); //v2.0.3 
    return; //v2.0.3 
}
...
function vtprd_show_rule_msgs_callback () {   //opt36   documentation only, no switches set here!   
  $allowed_html = vtprd_get_allowed_html(); //v2.0.3
    ?>
     <span><?php esc_attr_e('Pricing Deal Description Messages can be shown anywhere in Theme, both inside and outside the loop. ', 'vtprd');?>
        <strong><em><?php esc_attr_e('See', 'vtprd');?></em> =></strong>
     <a id="help36" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
     </span>
    <?php
    $html = ''; 
...
function vtprd_show_catalog_price_crossout_callback () {   //opt47
  ...
  $html .= '&nbsp;&nbsp;&nbsp;<em>' . __( 'Crossout shown ONLY at Product Catalog Display time (not in the cart)', 'vtprd' )  . '</em>&nbsp;&nbsp;&nbsp;'  ; 
    ...
    ?>
     <a id="help47" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';  
...
function vtprd_show_price_suffix_callback() {    //opt50
  ...
  $html = '<br><input type="text" class="" id="show_price_suffix"  rows="1" cols="100" name="vtprd_setup_options[show_price_suffix]" value="' . $options['show_price_suffix'] . '">';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help50" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
  $html .= '<p><em>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
...
function vtprd_show_yousave_one_some_msg_callback () {   //opt34
  ...
  $html .= '<option value="no"'  . selected( $options['show_yousave_one_some_msg'], 'no', false)  . '>'   . __('No', 'vtprd') . '</option>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help34" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';    
...
//	$html .= '<option value="none"'                   .  selected( $options['use_this_timeZone'], 'none', false)                    . '> &nbsp;&nbsp;' . __(' - Please Select the Store Time Zone - ', 'vtprd') . '</option>';
  ...
  $html .= '<option value="Kwajalein"'              .  selected( $options['use_this_timeZone'], 'Kwajalein', false)               . '>GMT-12 &nbsp;&nbsp; Kwajalein, Marshall Islands</option>';  
	$html .= '</select>';
  $url = esc_url("http://wwp.greenwichmeantime.com/time-zone/");  
  $html .= '&nbsp;&nbsp;&nbsp;<a  href="'.esc_url($url).'"  title="' . __('Find Your GMT Time Zone', 'vtprd') . '">' . __('Find Your GMT Time Zone', 'vtprd') . '</a>';
    
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help20" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';  
...
function vtprd_wholesale_products_display_callback() {   //opt54
  ...
  $html .= '<option value="retailAll"'     . selected( $options['wholesale_products_display'], 'retailAll', false)     . '>'   . __('Show All Products to Retail, Wholesale Products to Wholesale', 'vtprd') . '</option>'; //v1.1.1
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help54" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';  
  ...
  $html .= '<br><br>';
  $url = esc_url("https://wordpress.org/plugins/members/");
  $html .= '<a class="subsection-title-smaller" target="_blank" href="'.esc_url($url).'">Recomended Role Manager Plugin</a>'; 
...
function vtprd_wholesale_products_price_display_callback() {   //opt55
  ...
  $html .= '<option value="noPrices"'      . selected( $options['wholesale_products_price_display'], 'noPrices', false)      . '>'   . __('No Products Purchasable - use WooCommerce as a Catalog only', 'vtprd') . '</option>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help55" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';  
...
function vtprd_limit_cart_discounts_callback() {   //opt57
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help57" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_use_lifetime_max_limits_callback() {   //opt2
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help2" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_discount_floor_pct_per_single_item_callback() {    //opt3
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	
  $html = '<input type="text" class="smallText" id="discount_floor_pct_per_single_item"  rows="1" cols="20" style="width:50px;" name="vtprd_setup_options[discount_floor_pct_per_single_item]" value="' . $options['discount_floor_pct_per_single_item'] . '">' . '%';
    
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help3" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_discount_floor_pct_msg_callback() {    //opt4
  ...
  $html = '<textarea type="text" id="discount_floor_pct_msg"  rows="1" cols="60" name="vtprd_setup_options[discount_floor_pct_msg]">' . $options['discount_floor_pct_msg'] . '</textarea>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help4" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_show_checkout_discount_details_grouped_by_what_callback() {    //opt21                                 
  ...
  $html .= '</select>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help21" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = ''; 
...
function vtprd_show_cartWidget_discount_details_grouped_by_what_callback() {    //opt22                                 
  ...
  $html .= '</select></div>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOrCoupon help-anchor" id="help22" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOrCoupon">';
...
function vtprd_show_checkout_discount_titles_above_details_callback () {    //opt23
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help23" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_show_cartWidget_discount_titles_above_details_callback () {    //opt7
  ...
  $html = '<div class="unitPriceOrCoupon"><select id="show_cartWidget_discount_titles_above_details" name="vtprd_setup_options[show_cartWidget_discount_titles_above_details]">';  //v1.0.9.3
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOrCoupon help-anchor" id="help7" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOrCoupon">';
...
function vtprd_show_checkout_purchases_subtotal_callback () {    //opt24
  ...
  $html .= '<option value="beforeDiscounts"' . selected( $options['show_checkout_purchases_subtotal'], 'beforeDiscounts', false) . '>'   . __('Yes - Show ', 'vtprd') .  '&nbsp;'   . __('Before Discounts ', 'vtprd') .  '&nbsp;</option>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOrCoupon help-anchor" id="help24" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOrCoupon">';
...
function vtprd_show_cartWidget_purchases_subtotal_callback () {    //opt25
  ...
  $html .= '<option value="beforeDiscounts"' . selected( $options['show_cartWidget_purchases_subtotal'], 'beforeDiscounts', false) . '>'   . __('Yes - Show ', 'vtprd') .  '&nbsp;'   . __('Before Discounts ', 'vtprd') .  '&nbsp;</option>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOrCoupon help-anchor" id="help25" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOrCoupon">';
...
function vtprd_show_checkout_discount_total_line_callback () {    //opt5
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOrCoupon help-anchor" id="help5" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOrCoupon">';
...
function vtprd_show_cartWidget_discount_total_line_callback () {    //opt26
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOrCoupon help-anchor" id="help26" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOrCoupon">';
...
function vtprd_show_checkout_discount_detail_lines_callback () {    //opt6
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help6" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_show_cartWidget_discount_detail_lines_callback () {    //opt27
  ...
  $html = '<div class="unitPriceOrCoupon"> <select id="show_cartWidget_discount_detail_lines" name="vtprd_setup_options[show_cartWidget_discount_detail_lines]">'; //v1.0.9.3
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOrCoupon help-anchor" id="help27" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOrCoupon">';
...
function vtprd_debugging_mode_callback () {    //opt8
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help8" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_remove_extra_js_callback () {    //opt60
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help60" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';

  $html .= '<p id="help60-text" class = "help-text" >'; 
...
function vtprd_remove_extra_css_callback () {    //opt61
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help61" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';

  $html .= '<p id="help61-text" class = "help-text" >'; 
...
function vtprd_custom_checkout_css_callback() {    //opt9
  ...
  $html = '<textarea type="text" id="custom_checkout_css"  rows="200" cols="40" name="vtprd_setup_options[custom_checkout_css]">' . $options['custom_checkout_css'] . '</textarea>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help9" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_use_plugin_front_end_css_callback() {    //opt19                                               Use the Plugin CSS file for Discount Display
  ...
  $options = get_option( 'vtprd_setup_options' );
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help19" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_checkout_credit_detail_label_callback() {    //opt10
  ...
  $html = '<input type="text" class="smallText" id="checkout_credit_detail_label" name="vtprd_setup_options[checkout_credit_detail_label]" value="' . $options['checkout_credit_detail_label'] . '">';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a id="help10" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_checkout_credit_total_label_callback() {    //opt11
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	//<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
    $html = '<div class="unitPriceOrCoupon"> <input type="text" class="smallText" id="checkout_credit_total_label"  name="vtprd_setup_options[checkout_credit_total_label]" value="' . $options['checkout_credit_total_label'] . '"></div>';
    
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help11" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">';
    
  $html .= '<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
  $html .= '<i class="minusLineComment">'. __( '(suggested: "-" (minus sign) or "cr" (credit) )', 'vtprd' ) .'</i>';
...
function vtprd_cartWidget_credit_detail_label_callback() {    //opt28
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	
    $html = '<div class="unitPriceOrCoupon"> <input type="text" class="smallText" id="cartWidget_credit_detail_label" name="vtprd_setup_options[cartWidget_credit_detail_label]" value="' . $options['cartWidget_credit_detail_label'] . '"></div>'; //v1.0.9.3
    
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help28" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">';   
...
function vtprd_cartWidget_credit_total_label_callback() {    //opt29
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	 //<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
  $html = '<div class="unitPriceOrCoupon"> <input type="text" class="smallText" id="cartWidget_credit_total_label"  name="vtprd_setup_options[cartWidget_credit_total_label]" value="' . $options['cartWidget_credit_total_label'] . '"></div>';
   
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help29" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_checkout_credit_subtotal_title_callback() {    //opt30  
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	//<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
  $html = '<div class="unitPriceOrCoupon"> <input type="text" class="largeText" id="checkout_credit_detail_label" name="vtprd_setup_options[checkout_credit_subtotal_title]" value="' . $options['checkout_credit_subtotal_title'] . '"></div>';
   
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help30" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_checkout_credit_total_title_callback() {    //opt31
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	//<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
  $html = '<div class="unitPriceOrCoupon"> <input type="text" class="largeText" id="checkout_credit_total_title"  name="vtprd_setup_options[checkout_credit_total_title]" value="' . $options['checkout_credit_total_title'] . '"></div>';
   
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help31" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_checkout_new_subtotal_line_callback() {    //opt43
  ...
  $options = get_option( 'vtprd_setup_options' );  //<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
    ...
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help43" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_checkout_new_subtotal_label_callback() {    //opt44
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	//<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide 
  $html = '<div class="unitPriceOrCoupon"> <input type="text" class="largeText" id="checkout_new_subtotal_label" name="vtprd_setup_options[checkout_new_subtotal_label]" value="' . $options['checkout_new_subtotal_label'] . '"></div>';
   
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help44" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_cartWidget_new_subtotal_line_callback() {    //opt45
  ...
  $options = get_option( 'vtprd_setup_options' );   //<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
    ...
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help45" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_cartWidget_new_subtotal_label_callback() {    //opt46
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	 //<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
  $html = '<div class="unitPriceOrCoupon"> <input type="text" class="largeText" id="cartWidget_new_subtotal_label" name="vtprd_setup_options[cartWidget_new_subtotal_label]" value="' . $options['cartWidget_new_subtotal_label'] . '"></div>';
   
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help46" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_cartWidget_credit_subtotal_title_callback() {    //opt32
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	 //<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
  $html = '<div class="unitPriceOrCoupon"> <input type="text" class="mediumText" id="cartWidget_credit_detail_label" name="vtprd_setup_options[cartWidget_credit_subtotal_title]" value="' . $options['cartWidget_credit_subtotal_title'] . '"></div>';
   
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help32" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_cartWidget_credit_total_title_callback() {    //opt33
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	 //<span class="unitPriceOrCoupon">  added v1.0.9.0 , is accessed for show/hide
  $html = '<div class="unitPriceOrCoupon"> <input type="text" class="mediumText" id="cartWidget_credit_total_title"  name="vtprd_setup_options[cartWidget_credit_total_title]" value="' . $options['cartWidget_credit_total_title'] . '"></div>';
   
    echo wp_kses($html ,$allowed_html );    
    ?>
     <span class="unitPriceOrCoupon"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a id="help33" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    </span>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_lifetime_limit_by_user_callback () {   //opt64
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help64" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_lifetime_limit_by_ip_callback () {   //opt13
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help13" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_lifetime_limit_by_email_callback () {   //opt14
  ...
  $html .= '<option value="yes"' . selected( $options['max_purch_rule_lifetime_limit_by_email'], 'yes', false) . '>'   . __('Yes', 'vtprd') .  '&nbsp;</option>';  
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help14" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_lifetime_limit_by_billto_name_callback () {   //opt15
  ...
  $html .= '<option value="yes"' . selected( $options['max_purch_rule_lifetime_limit_by_billto_name'], 'yes', false) . '>'   . __('Yes', 'vtprd') .  '&nbsp;</option>';  
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help15" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_lifetime_limit_by_billto_addr_callback () {   //opt16
  ...
  $html .= '<option value="yes"' . selected( $options['max_purch_rule_lifetime_limit_by_billto_addr'], 'yes', false) . '>'   . __('Yes', 'vtprd') .  '&nbsp;</option>';	
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help16" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_lifetime_limit_by_shipto_name_callback () {   //opt17
  ...
  $html .= '<option value="yes"' . selected( $options['max_purch_rule_lifetime_limit_by_shipto_name'], 'yes', false) . '>'   . __('Yes', 'vtprd') .  '&nbsp;</option>';	
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help17" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_lifetime_limit_by_shipto_addr_callback () {   //opt18
  ...
  $html .= '<option value="yes"' . selected( $options['max_purch_rule_lifetime_limit_by_shipto_addr'], 'yes', false) . '>'   . __('Yes', 'vtprd') .  '&nbsp;</option>';	
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help18" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_before_checkout_products_selector_callback() {    //opt39
  ...
  $html = '<textarea type="text" id="show_error_before_checkout_products_selector"  rows="1" cols="20" name="vtprd_setup_options[show_error_before_checkout_products_selector]">' . $options['show_error_before_checkout_products_selector'] . '</textarea>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help39" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
  ...
  $html .= __('"Show Error Messages Just Before Checkout Products List - HTML Selector" => 
        ...
        .'<br><br>'. __('Default = "', 'vtprd') .esc_url(VTPRD_CHECKOUT_PRODUCTS_SELECTOR_BY_PARENT) . '".'   
...
function vtprd_before_checkout_address_selector_callback() {    //opt40
  ...
  $html = '<textarea type="text" id="show_error_before_checkout_address_selector"  rows="1" cols="20" name="vtprd_setup_options[show_error_before_checkout_address_selector]">' . $options['show_error_before_checkout_address_selector'] . '</textarea>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help40" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';  
  ...
  $html .= __('"Show Error Messages Just Before Checkout Address  List - HTML Selector" => 
        This option controls the location of the message display.', 'vtprd') 
        .'<br><br>'. __('Blank = do not use. ', 'vtprd')
        .'<br><br>'. __('Default = "', 'vtprd') .esc_url(VTPRD_CHECKOUT_ADDRESS_SELECTOR_BY_PARENT) . '".'   
        .'<br><br>'. __('If you"ve changed this value and can"t get it to work, you can use the "reset to defaults" button (just below the "save changes" button) to get the value back (snapshot your other settings first to help you quickly set the other settings back the way to what you had before.)', 'vtprd');   
  $html .= '</p>';   
  $html .= '<br><em>&nbsp;&nbsp;';
...
function vtprd_lifetime_purchase_button_error_msg_callback() {    //opt41
  ...
  $tempMsg =    str_replace(array("\r\n", "\r", "\n", "\t"), ' ', $options['lifetime_purchase_button_error_msg'] ?? '' );    //v2.0.3 added  ?? '' 
  ...
  $html = '<textarea type="text" id="lifetime_purchase_button_error_msg"  rows="200" cols="40" name="vtprd_setup_options[lifetime_purchase_button_error_msg]">' . $options['lifetime_purchase_button_error_msg'] . '</textarea>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help41" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';
...
function vtprd_discount_taken_where_callback () {    //opt48
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help48" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>">More Info</a>
    <?php
    $html = '';
...
function vtprd_give_more_or_less_discount_callback () {    //opt49
  ...
  $html .= '&nbsp;&nbsp;&nbsp;<em>' . __( '(if the discount does not divide out equally)', 'vtprd' )  . '</em>&nbsp;&nbsp;&nbsp;</div>'  ; 
     
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOnly help-anchor" id="help49" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOnly">'; 
...
function vtprd_coupon_discount_coupon_name_callback() {    //opt58
  ...
  //error_log( print_r(  'vtprd_woo_maybe_create_coupon_types SETUP OPTIONS', true ) );         
    ...
    $html = '<div class="unitPriceOrCoupon"><br><input type="text" class="" id="coupon_discount_coupon_name"  rows="1" cols="100" name="vtprd_setup_options[coupon_discount_coupon_name]" value="' . $options['coupon_discount_coupon_name'] . '"></div>';
    
    echo wp_kses($html ,$allowed_html );    
    ?>
        <a class="unitPriceOrCoupon help-anchor" id="help58"href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOrCoupon">'; 
...
function vtprd_show_woo_coupon_cart_totals_00_line_callback () {   //opt59
  ...
  $html .= '<option value="no"'  . selected( $options['show_woo_coupon_cart_totals_00_line'], 'no', false)  . '>'   . __('No', 'vtprd') . '</option>';
    ...
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a id="help59" class="help-anchor" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '';    
...
function vtprd_show_unit_price_cart_discount_crossout_callback () {    //opt51
	...
	$html .= '</select>'; 
  $html .= '&nbsp;&nbsp;&nbsp;<em>' . __( '(Cart Discount Only)', 'vtprd' )  . '</em>&nbsp;&nbsp;&nbsp; </div>'  ; 
    
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOnly help-anchor" id="help51" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOnly">';
...
function vtprd_show_unit_price_cart_discount_computation_callback () {    //opt52
  ...
  $html .= '&nbsp;&nbsp;&nbsp;<em>' . __( '(For Testing only)', 'vtprd' )  . '</em>&nbsp;&nbsp;&nbsp; </div>'  ; 
    
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOnly help-anchor" id="help52" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOnly">';
...
function vtprd_unit_price_cart_savings_message_callback() {    //opt53
	$allowed_html = vtprd_get_allowed_html(); //v2.0.3
    $options = get_option( 'vtprd_setup_options' );	
  $html = '<div class="unitPriceOnly"> <input type="text" class="" id="unit_price_cart_savings_message"  rows="1" cols="100" name="vtprd_setup_options[unit_price_cart_savings_message]" value="' . $options['unit_price_cart_savings_message'] . '"></div>';
    
    echo wp_kses($html ,$allowed_html );    
    ?>
     <a class="unitPriceOnly help-anchor" id="help53" href="<?php echo esc_js('javascript:void(0);');?>"><?php esc_attr_e('More Info', 'vtprd');?></a>
    <?php
    $html = '<div class="unitPriceOnly">';
...
function vtprd_check_memory_limit() {    
    ...
    if (function_exists('wc_let_to_num')) {
      ...
      $memory = wc_let_to_num( WP_MEMORY_LIMIT );
  		...
  		if ( $memory < 67108864 ) {
  		     $url = esc_url("http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP");
             $message = '<strong>WP Memory Limit: ' . sprintf( __( '%s - We recommend setting memory to at least 64MB. See: <a href="%s">Increasing memory allocated to PHP</a>', 'woocommerce' ), size_format( $memory ), $url ) .'</strong>';
           $allowed_html = vtprd_get_allowed_html(); //v2.0.3
           echo wp_kses($message ,$allowed_html ); //v2.0.3
...
function vtprd_destroy_session() {    
    ...
    //error_log( print_r(  'session_destroy in vtprd_destroy_session setup-options ', true ) );
    ...
    session_write_close(); //v2.0.2.02b
    return;
}
//v1.0.9.0 end
  ...
  function vtprd_enqueue_setup_scripts($hook_suffix) {
    ...
    switch( $hook_suffix) {        //weird but true
      ...
      case 'vtprd-rule_page_vtprd_show_faq_page': 
        ...
        wp_register_style('vtprd-admin-settings-style', esc_url(VTPRD_URL.'/admin/css/vtprd-admin-settings-style.css') );  
        wp_enqueue_style ('vtprd-admin-settings-style');
        wp_register_script('vtprd-admin-settings-script', esc_url(VTPRD_URL.'/admin/js/vtprd-admin-settings-script-v003.js') );  
    ...
    switch( $hook_suffix) {        //weird but true
      case 'vtprd-rule_page_vtprd_setup_options_page':
             $vtprd_inline_style = '  
                  /* To fix the oddly performing "MORE INFO" buttons */
                  #help58,
                  #help30,
                  #help31,
                  #help11,
                  ...
                  /* i comment lines weirdly spaced fix*/
                  .minusLineComment
                  { 
                  float: left;
                  margin-top: -40px;
                  margin-left: 7%;
                        ...
                        ';
              wp_add_inline_style('vtprd-admin-style',$vtprd_inline_style );  //v2.0.3 
        break;
      case 'vtprd-rule_page_vtprd_show_help_page':
        ...
        /* inline updates are no longer 2-step, but done under the table realtime.
        ***if css needed, refer to older generations***
        if ($doThis ==  'reportInclExclV2.0.0'){
            $vtprd_inline_style = '    
             ';
            wp_add_inline_style('vtprd-admin-style',$vtprd_inline_style );  //v2.0.3 
...
function vtprd_validate_setup_input( $input ) {
   //error_log( print_r(  'begin function vtprd_validate_setup_input ', true ) );
   //error_log( print_r(  'Input= ', true ) );
   //error_log( var_export($input, true ) );  

    //*****************************************************
    //v2.0.3 begin - ** sanitize input **  using wp_kses 
  	//*****************************************************    
      $allowed_html = vtprd_get_allowed_html(); //v2.0.3
      foreach( $input as $key => $value ) {
  		if( isset( $input[$key] ) ) {
            $input[$key] = wp_kses($input[$key],$allowed_html);
  		} 	
  	  } // end foreach        
    //v2.0.3 end
    //*****************************************************
  ...
  $vtprd_setup_options = $output; //v2.0.3 prevent warnings
  ...
  //error_log( print_r(  'deleted $vtprd_license_options', true ) );  
    ...
    case $deleteALL    === true :    //set delete all on plugin delete
        $this->vtprd_set_to_deleteALL_on_UnInstall();  //set uninstall to delete all   //v2.0.3
      break;

    case $UNdeleteALL    === true :    //unset delete all
        $this->vtprd_unset_deleteALL();  //set uninstall to delete all  //v2.0.3       
      break;
    //v2.0.2.0 end
  ...
  if (isset($input['discount_floor_pct_per_single_item'])) {
    $input['discount_floor_pct_per_single_item'] = preg_replace( '/[^0-9.]+/', '', $input['discount_floor_pct_per_single_item'] ?? '' ); //v2.0.3 added  ?? ''  //remove leading/trailing spaces, percent sign, dollar sign
  }
   
  if (isset($input['lifetime_purchase_button_error_msg'])) {
    $tempMsg =    str_replace(array("\r\n", "\r", "\n", "\t"), ' ', $input['lifetime_purchase_button_error_msg'] ?? '' );   //v2.0.3 added  ?? '' 

...
/*
***********************************
v2.0.3 global change -
in all 'error_msg' fields, all double quotes were replaced with a dash
==> this enables double quotes to be used successfully to wrap the <div> containing the msg,
==> **without creating a JS error when using inline scripts as now required.
***********************************
*/
...
class VTPRD_Rule_update {
    ...
    //********     
    //v2.0.3 begin - grab a copy of the rule before any screen handling, in case of Gutenberg-induced error below
    //********   
      //$vtprd_rules_set = get_option('vtprd_rules_set'); //$vtprd_rules_set not global, just for this access
      //v2.1.0 BEGIN
      /*
      if (!$vtprd_rules_set) {
        $sizeof_rules_set = 0;
      ...
      */
      ...
      for($i=0; $i < $sizeof_rules_set; $i++) {       
         if ($vtprd_rules_set[$i]->post_id == $post->ID) {
            $_SESSION['vtprd_save_current_rule'] = $vtprd_rules_set[$i];
            $i =  $sizeof_rules_set;
         }
      }
      */
  ...
  ****************************************************************************************/ 
  ...
  public  function vtprd_edit_rule() {
      ...
      global $vtprd_edit_arrays_framework;   //v2.0.3 
      $save_edit_arrays_framework =  $vtprd_edit_arrays_framework; //v2.0.3 hold orig values for later pass at update time
     ...
     if ( (vtprd_sanitize_text_or_array_field($_REQUEST['post_title']) > ' ' ) ) {
       //do nothing
     } else {     
...
*/
      ...
      $vtprd_rule->cart_or_catalog_select   = vtprd_sanitize_text_or_array_field($_REQUEST['cart-or-catalog-select']);  
      $vtprd_rule->pricing_type_select      = vtprd_sanitize_text_or_array_field($_REQUEST['pricing-type-select']);  
      $vtprd_rule->minimum_purchase_select  = vtprd_sanitize_text_or_array_field($_REQUEST['minimum-purchase-select']);  
 
      $vtprd_rule->rule_on_off_sw_select    = vtprd_sanitize_text_or_array_field($_REQUEST['rule-on-off-sw-select']);
      $vtprd_rule->rule_type_select         = vtprd_sanitize_text_or_array_field($_REQUEST['rule-type-select']);
      //$vtprd_rule->wizard_on_off_sw_select  = $_REQUEST['wizard-on-off-sw-select']; //v2.0.0 removed!!
      
      $vtprd_rule->apply_deal_to_cheapest_select  = vtprd_sanitize_text_or_array_field($_REQUEST['apply-deal-to-cheapest-select']); //v.1.6.7
              
      $upperSelectsDoneSw                   = vtprd_sanitize_text_or_array_field($_REQUEST['upperSelectsDoneSw']); 
     ...
     $vtprd_rule->rule_template = vtprd_sanitize_text_or_array_field($_REQUEST['rule_template_framework']); 
     ...
     for($k=0; $deal_iterations_done == 'no'; $k++) {      
       ...
       if ( (isset($_REQUEST['buy_repeat_condition_' . $k])) && (!empty( vtprd_sanitize_text_or_array_field($_REQUEST['buy_repeat_condition_' . $k]) )) ) {    //is a deal line there? always 1 at least...
         foreach( $vtprd_deal_structure_framework as $key => $value ) {   //spin through all of the screen fields=>  $key = field name, so has multiple uses...  
            ...
            //v1.1.8.1 new isset
            $thiskey = $key . '_' .$k; //v2.0.3
            if (isset($_REQUEST[$thiskey]) ) {
              $vtprd_deal_structure_framework[$key] = vtprd_sanitize_text_or_array_field($_REQUEST[$thiskey]);
            }             
         } 
     ...
     */
      ...
      //*********************
      //v2.0.3 begin
      //NEW FIRST TIME CATALOG RULES SEEM TO HAVE AN UPDATE PROBLEM THAT APPEARS ONLY ONCE - FIX BELOW  if ($sizeof_info == 0)
      //*********************
 
      $sizeof_info = is_array($vtprd_rule->rule_deal_info) ? sizeof($vtprd_rule->rule_deal_info) : 0; //v2.1.0  
      //if (sizeof($vtprd_rule->rule_deal_info) == 0) {  //all the lower screen data has been lost due to JS conflict
      if ($sizeof_info == 0) {  //all the lower screen data has been lost due to JS conflict
        //reset the poorly received rule with saved original, if there.
        
        $vtprd_rule->rule_deal_info[0] =
            array (
              'buy_repeat_condition' => 'none',
              'buy_repeat_count' => '',
              'buy_amt_type' => 'none',
              'buy_amt_count' => '',
              'buy_amt_applies_to' => 'all',
              'buy_amt_mod' => 'none',
              'buy_amt_mod_count' => '',
              'action_repeat_condition' => 'none',
              'action_repeat_count' => '',
              'action_amt_type' => 'none',
              'action_amt_count' => '',
              'action_amt_applies_to' => 'all',
              'action_amt_mod' => 'none',
              'action_amt_mod_count' => '',
              'discount_amt_type' => '0',
              'discount_amt_count' => '',
              'discount_auto_add_free_product' => '',
              'discount_applies_to' => 'each',
              'discount_rule_max_amt_type' => 'none',
              'discount_rule_max_amt_count' => '',
              'discount_lifetime_max_amt_type' => 'none',
              'discount_lifetime_max_amt_count' => '',
              'discount_rule_cum_max_amt_type' => 'none',
              'discount_rule_cum_max_amt_count' => '', 
              );
  
          $vtprd_rule->rule_deal_info[0]['discount_amt_type'] = vtprd_sanitize_text_or_array_field($_REQUEST['discount_amt_type_0']);
          $vtprd_rule->rule_deal_info[0]['discount_amt_count']  = vtprd_sanitize_text_or_array_field($_REQUEST['discount_amt_count_0']);
          
          $vtprd_rule->buy_group_population_info = $save_edit_arrays_framework['buy_group_framework']; 
          $vtprd_rule->action_group_population_info = $save_edit_arrays_framework['action_group_framework']; 
      ...
      /*
      if (isset($_SESSION['vtprd_save_current_rule'])) {
        unset( $_SESSION['at_login_check_saved_cart_in_housekeeping'], $old_current_rule );
      }
      */
      //v2.0.3 end 
     ...
     //inPop        
     $vtprd_rule->inPop = vtprd_sanitize_text_or_array_field($_REQUEST['popChoiceIn']);
     /* v2.0.0
        popChoiceIn:
          wholeStore
          groups     
     */
     ...
     $vtprd_rule->actionPop = vtprd_sanitize_text_or_array_field($_REQUEST['popChoiceOut']);
     /* v2.0.0
      popChoiceOut: 
        sameAsInPop
        wholeStore
     ...
     */
     ...
     if ($vtprd_rule->pricing_type_select == 'bulk') {
        ...
        $vtprd_rule->bulk_deal_method = vtprd_sanitize_text_or_array_field($_REQUEST['bulkMethodIn']);
        $vtprd_rule->bulk_deal_qty_count_by = vtprd_sanitize_text_or_array_field($_REQUEST['bulkCountByIn']);
        ...
        if(!empty( vtprd_sanitize_text_or_array_field($_REQUEST['minVal']) )) { 
          $minVal = vtprd_sanitize_text_or_array_field($_REQUEST['minVal']);
          $maxVal = vtprd_sanitize_text_or_array_field($_REQUEST['maxVal']);
          $discountType = vtprd_sanitize_text_or_array_field($_REQUEST['discountType']);
          $discountVal = vtprd_sanitize_text_or_array_field($_REQUEST['discountVal']);
    ...
    $vtprd_rule->discount_rule_max_amt_msg = vtprd_sanitize_text_or_array_field($_REQUEST['discount_rule_max_amt_msg']);

    //if max_lifetime_amt_type is active, may have a max_amt_msg
    $vtprd_rule->discount_lifetime_max_amt_msg = vtprd_sanitize_text_or_array_field($_REQUEST['discount_lifetime_max_amt_msg']);

    //if max_cum_amt_type is active, may have a max_amt_msg
    $vtprd_rule->discount_rule_cum_max_amt_msg = vtprd_sanitize_text_or_array_field($_REQUEST['discount_rule_cum_max_amt_msg']);
    
               
    $vtprd_rule->discount_product_short_msg = vtprd_sanitize_text_or_array_field($_REQUEST['discount_product_short_msg']);
    if ( ($vtprd_rule->discount_product_short_msg <= ' ') || 
         ($vtprd_rule->discount_product_short_msg == vtprd_sanitize_text_or_array_field($_REQUEST['shortMsg']) ) ) {
        if ($vtprd_rule->cart_or_catalog_select == 'cart') {    //V2.0.3 ONLY CARE ABOUT THIS IN CART PROCESSING, NOT RELEVANT IN CATALOG PROCESSING
            $vtprd_rule->rule_error_message[] = array( 
    ...
    $vtprd_rule->discount_product_full_msg = vtprd_sanitize_text_or_array_field($_REQUEST['discount_product_full_msg']); 
    ...
    $vtprd_rule->cumulativeRulePricing = vtprd_sanitize_text_or_array_field($_REQUEST['cumulativeRulePricing']); 
    if ($vtprd_rule->cumulativeRulePricing == 'yes') {
       if ($vtprd_rule->cumulativeRulePricingAllowed == 'yes') {
         $vtprd_rule->ruleApplicationPriority_num = vtprd_sanitize_text_or_array_field($_REQUEST['ruleApplicationPriority_num']);
         $vtprd_rule->ruleApplicationPriority_num = preg_replace('/[^0-9.]+/', '', $vtprd_rule->ruleApplicationPriority_num ?? '' ); //v2.1.0  //remove leading/trailing spaces, percent sign, dollar sign  //v2.0.3 added  ?? '' 
         if ( is_numeric($vtprd_rule->ruleApplicationPriority_num) === false ) { 
            ...
            $vtprd_rule->rule_error_message[] = array( 
                  ...
                  'error_msg'  => __('With this Rule Template chosen, -Apply this Rule Discount in Addition to Other Rule Discounts- must = -No-.', 'vtprd') );  //v2.0.3
    ...
    $vtprd_rule->cumulativeSalePricing   = vtprd_sanitize_text_or_array_field($_REQUEST['cumulativeSalePricing']);
    if ( ($vtprd_rule->cumulativeSalePricing != 'no') && ($vtprd_rule->cumulativeSalePricingAllowed == 'no') ) {
      $vtprd_rule->rule_error_message[] = array( 
            'insert_error_before_selector' => '#cumulativePricing_box',  
            'error_msg'  => __('With this Rule Template chosen, -Rule Discount in addition to Product Sale Pricing- must = -Does not apply when Product Sale Priced-.', 'vtprd') );   //v2.0.3
      $vtprd_rule->rule_error_red_fields[] = '#cumulativePricing_box';
      $vtprd_rule->rule_error_box_fields[] = '#cumulativePricing';  
    }
               
    $vtprd_rule->cumulativeCouponPricing = vtprd_sanitize_text_or_array_field($_REQUEST['cumulativeCouponPricing']);            
    if ( ($vtprd_rule->cumulativeCouponPricing == 'yes') && ($vtprd_rule->cumulativeCouponPricingAllowed == 'no') ) {
      $vtprd_rule->rule_error_message[] = array( 
            'insert_error_before_selector' => '#cumulativePricing_box',  
            'error_msg'  => __('With this Rule Template chosen, -Apply Rule Discount in addition to Coupon Discount?- must = -No-.', 'vtprd') );   //v2.0.3
      $vtprd_rule->rule_error_red_fields[] = '#cumulativePricing_box' ;
      $vtprd_rule->rule_error_box_fields[] = '#cumulativePricing'; 
    } 
     ...
     //v1.1.8.0 end
      ...
      //********************************************************************************************************************
       ...
       $vtprd_rule->only_for_this_coupon_name   = vtprd_sanitize_text_or_array_field($_REQUEST['only_for_this_coupon_name']);
        ...
        for($b=0; $b < $row_count; $b++) {
          ...
          */  
          if ($vtprd_rule->bulk_deal_method == 'currency') { //currency pre-process            
            ...
            $vtprd_rule->bulk_deal_array[$b]['min_value'] = str_replace($thousand_separator, '', $vtprd_rule->bulk_deal_array[$b]['min_value']  ?? '');    //v2.0.3 added  ?? '' 
            $vtprd_rule->bulk_deal_array[$b]['max_value'] = str_replace($thousand_separator, '', $vtprd_rule->bulk_deal_array[$b]['max_value']  ?? '');     //v2.0.3 added  ?? '' 
            //v2.0.0.8 begin
            //remove spaces which might be used as an optional thousands separator
            $vtprd_rule->bulk_deal_array[$b]['min_value'] = str_replace(' ', '', $vtprd_rule->bulk_deal_array[$b]['min_value']  ?? '');  //v2.0.3 added  ?? '' 
            $vtprd_rule->bulk_deal_array[$b]['max_value'] = str_replace(' ', '', $vtprd_rule->bulk_deal_array[$b]['max_value']  ?? '');  //v2.0.3 added  ?? '' 
            //v2.0.0.8 end
            
            if ($decimal_separator == ',') {
              $vtprd_rule->bulk_deal_array[$b]['min_value'] = str_replace($decimal_separator, '.', $vtprd_rule->bulk_deal_array[$b]['min_value']  ?? '');   //v2.0.3 added  ?? '' 
              $vtprd_rule->bulk_deal_array[$b]['max_value'] = str_replace($decimal_separator, '.', $vtprd_rule->bulk_deal_array[$b]['max_value']  ?? '');    //v2.0.3 added  ?? '' 
            }
            if ( ($vtprd_rule->bulk_deal_array[$b]['min_value'] > ' ') && //min_value blank replaced by 0.00 later
               ...
               $num_decimals = strlen(preg_replace("/.*\./", "", $vtprd_rule->bulk_deal_array[$b]['min_value'] ?? '' )); //v2.0.3 added  ?? '' 
               if ( ($num_decimals !== 2) ||
            ...
            if ( ($vtprd_rule->bulk_deal_array[$b]['max_value'] > ' ') && //may be blank, tested if appropriate later...
               ...
               $num_decimals = strlen(preg_replace("/.*\./","", $vtprd_rule->bulk_deal_array[$b]['max_value'] ?? '' ));  //v2.0.3 added  ?? ''  
               if ( ($num_decimals !== 2) ||
          ...
          $vtprd_rule->bulk_deal_array[$b]['discount_value'] = str_replace($thousand_separator, '', $vtprd_rule->bulk_deal_array[$b]['discount_value']  ?? '' );  //v2.0.3 added  ?? ''      //ALWAYS do this!
          //remove spaces which might be used as an optional thousands separator
          $vtprd_rule->bulk_deal_array[$b]['discount_value'] = str_replace(' ', '', $vtprd_rule->bulk_deal_array[$b]['discount_value']  ?? '' );  //v2.0.3 added  ?? ''      //ALWAYS do this!          
...
/*      
  ...
  */
    ...
    //*****************************************
    $sizeof_message = is_array($vtprd_rule->rule_error_message) ? sizeof($vtprd_rule->rule_error_message) : 0; //v2.1.0
...
*/
    ...
    if(!isset($_SESSION)){   //v2.0.2.02
    //if (session_id() == "") {    //v2.0.2.02
      session_start();    
    } 
    $_SESSION = array();
    ...
    //error_log( print_r(  '$_SESSION destroy in rules update', true ) );
  ...
  public function vtprd_validate_rule_scheduling() {
    ...
    for($t=0; $loop_ended  == 'no'; $t++) {
      ...
      if ( (isset($_REQUEST['date-begin-' .$t])) ||
        ...
        $date = vtprd_sanitize_text_or_array_field( $_REQUEST['date-begin-' .$t] );
        ...
        $date = vtprd_sanitize_text_or_array_field( $_REQUEST['date-end-' .$t] );
      ...
      if ( $fieldAttributes['edit_is_active'] ) {   //if field active for this template selection
        ...
        switch( $fieldAttributes['field_type'] ) {
          ...
          case 'amt':   //amt is ALWAYS preceeded by a dropdown of some sort...
              ...
              $vtprd_deal_structure_framework[$fieldName] =  preg_replace('/[^0-9.]+/', '', $vtprd_deal_structure_framework[$fieldName] ?? ''); //v2.0.3 added  ?? ''  //remove leading/trailing spaces, percent sign, dollar sign
                 ...
                 if ( (isset($vtprd_deal_structure_framework[$cross_field_name])) && 
                    ...
                    if ( ($fieldName = 'discount_auto_add_free_product') &&
                        ...
                        if ($cross_field_name == 'popChoiceOut') {
                          $field_value_temp = vtprd_sanitize_text_or_array_field( $_REQUEST['popChoiceOut'] );
                        } else {
                          $field_value_temp = vtprd_sanitize_text_or_array_field( $_REQUEST['cumulativeCouponPricing'] );
                        }
  ...
  public function vtprd_dump_deal_lines_to_rule() {  
     ...
     for($k=0; $deal_iterations_done == 'no'; $k++) {      
       $thiskey = $key . '_' .$k; //v2.0.3
       if ( (isset( $_REQUEST[$thiskey ] )) && (!empty( $_REQUEST['buy_repeat_condition_' . $k] )) ) {    //is a deal line there? always 1 at least...
      ...
      if ($vtprd_rule->inPop == 'groups') {        
        ...
        foreach( $buy_arrays_framework as $key => $value ) {  
           if (isset($_REQUEST[$key])) {
             ...
             //error_log( print_r(  'buy_group_framework location = ', true ) ); 
             //error_log( var_export($vtprd_edit_arrays_framework['buy_group_framework'][$key], true ) );
             //error_log( print_r(  'input before sanitization = ' .$_REQUEST[$key], true ) );
             //error_log( var_export(  $_REQUEST[$key] , true ) );
             //$sanArray = vtprd_sanitize_text_or_array_field($_REQUEST[$key]);
             //error_log( print_r(  'input after sanitization  = ', true ) );
             //error_log( var_export(  $sanArray , true ) );
           
             //vtprd_edit_arrays_framework['buy_group_framework'][$key] = vtprd_sanitize_text_or_array_field( $_REQUEST[$key] );   - doesn't work, sanitize can't work with ARRAYS
             // $_REQUEST[$key] is an ARRAY
                          
             $vtprd_edit_arrays_framework['buy_group_framework'][$key] = vtprd_sanitize_text_or_array_field($_REQUEST[$key]);
    ...
    //error_log( var_export($vtprd_edit_arrays_framework['buy_group_framework'][$key], true ) );
        ...
        if ($buy_groups_found == 0) {
            $vtprd_rule->rule_error_message[] = array(     
              ...
              'error_msg'  => '-By Category / Role / Product- chosen above, <br><br>but <strong>no selections made in the boxes below</strong>.&nbsp;&nbsp;&nbsp; <br><br>Please make a selection in either the -Select Products- box or -Select Customers- box below. &nbsp;&nbsp;&nbsp;'   //v2.0.3
              ); 
            $vtprd_rule->rule_error_box_fields[] = '.buy-group-select-product-area';
      ...
      */ 
      ...
      if ($vtprd_rule->actionPop == 'groups') {     
        ...
        foreach( $action_arrays_framework as $key => $value ) {  
           ...
           if (isset($_REQUEST[$key])) {
              $vtprd_edit_arrays_framework['action_group_framework'][$key] = vtprd_sanitize_text_or_array_field( $_REQUEST[$key] );
  ...
  public function vtprd_build_varName_array($varName_string) {
    ...
    foreach ($varName_exploded as $varName) {
      ...
      foreach ($varName_exploded_combo as $varName_combo) {
        $varName_array_combo[] = strtolower(trim($varName_combo  ?? '' ));  //remove leading or trailing spaces, make lower case...      //v2.0.3 added  ?? '' 
      }              
